...
try{
 ...
 for(const [id,file] of [['zona-de-perreo','zona-de-perreo.nflyer']]){
   ...
   for(const format of ['square','story']){
     ...
     for(const value of [40,100,200,300]) {
       if(!await size.isVisible()) await title.click();
       await size.fill(String(value));
       console.log('SIZE',format,value);
       await page.waitForTimeout(1800);
       assert.equal(await node.evaluate(el=>getComputedStyle(el).scale),"1","manual size must not be visually scaled down");
       assert.equal(await node.evaluate(el=>getComputedStyle(el).translate),"0px","manual size must not be shifted by containment");
