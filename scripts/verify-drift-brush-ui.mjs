...
const glyphFamily='Drift Brush SVG';
...
const out = process.env.NF_OUTPUT_DIR || '/tmp/drift-brush-ui';
...
try{
 ...
 for(const [,file] of [['glyph-check',process.env.NF_PROJECT || 'day-party.nflyer']]){
   ...
   for(const [objectId,panel,placeholder] of [['headline','headline','ENTER HEADLINE...'],['subtitle','head2','Optional sub-headline']]){
     ...
     const option=page.getByRole('button',{name:/Drift Brush SVG/}).last();
     await option.screenshot({path:out+'/'+panel+'-picker.png'});
     await option.click();
   ...
   assert.equal(errors.length,0,JSON.stringify(errors));console.log('SVG FAMILY SELECTION, EDITING AND ROUNDTRIP PASSED');
 }
