...
const out=process.env.NF_BEAT_AUDIT_DIR || '/tmp/beat-therapy';
const projectPath=process.env.NF_BEAT_IMPORT || 'public/generated-flyers/beat-therapy.nflyer';
...
if(process.env.NF_BEAT_EXPORT_ACCESS_FIXTURE==='1') {
  ...
  for(const format of ['square','story']) {
    ...
    await page.waitForFunction(({format,headline})=>{
      ...
      const ready=b&&(format==='square'?Math.abs(b.width/b.height-1)<.01:b.height/b.width>1.7)
        &&root.querySelectorAll('[data-coco-text-value]').length>=21
      ...
      if(!ready){window.__beatReadyAt=0;return false;}
      window.__beatReadyAt ||= Date.now();
      return Date.now()-window.__beatReadyAt>1000;
    ...
    assert.equal(objects.filter(o=>o.text).length,21);
    ...
    if(process.env.NF_BEAT_PUBLISH_PREVIEWS==='1')await page.locator('#artboard').screenshot({path:`public/generated-flyers/beat-therapy-${format}-preview.png`});
    await captureStyle.evaluate(node=>node.remove());
    console.log('PASS',format,objects.length,'text owners');
  }
  if(process.env.NF_BEAT_EXPORT_CHECK==='1') {
    ...
    for(const format of ['square','story']) {
      ...
      await page.waitForFunction(format=>{
        const root=document.querySelector('#artboard'),b=root?.getBoundingClientRect();
        return b&&(format==='square'?Math.abs(b.width/b.height-1)<.01:b.height/b.width>1.7)&&root.querySelectorAll('[data-coco-text-value]').length>=21;
