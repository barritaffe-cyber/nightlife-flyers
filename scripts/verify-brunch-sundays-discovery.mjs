...
const out = process.env.NF_OUTPUT_DIR || '/tmp/brunch-sundays-discovery';
const eventName = process.env.NF_EVENT_NAME || 'Brunch Sundays';
const theme = process.env.NF_THEME || 'Brunch';
const recipeId = process.env.NF_RECIPE_ID || 'brunch-sundays';
...
try {
  ...
  console.log('PASS Brunch Sundays / Brunch through both personalized previews to the details form.');
} catch (error) {
