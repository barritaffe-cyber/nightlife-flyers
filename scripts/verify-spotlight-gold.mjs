...
const glyphFamily='Spotlight Gold PNG';
const sampleText = 'Gold\nLive';
const out = process.env.NF_OUTPUT_DIR || '/tmp/spotlight-gold-check';
...
try{
 await page.goto(`${baseUrl}/generated-flyers/spotlight-gold-font-preview.html`);
 ...
 for(const [id,file] of [['glyph-check',process.env.NF_PROJECT || 'ycee-live.nflyer']]){
   ...
   for(const [objectId,panel,placeholder] of [['headline','headline','ENTER HEADLINE...'],['subtitle','head2','Optional sub-headline']]){
     ...
     for(const [id,panel,placeholder,text,size] of [
       ['headline','headline','ENTER HEADLINE...','Live Gold','48'],
   ...
   assert.equal(errors.length,0,JSON.stringify(errors));console.log('SPOTLIGHT GOLD: BOTH PICKERS, FORMATS, EDITING, ROUNDTRIP, AND PNG EXPORT PASSED');
 }
