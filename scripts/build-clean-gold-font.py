"""Extract supplied Clean Gold sheets and package editable UI bitmap lettering.

Run with /tmp/offshore-font-build/bin/python scripts/build-clean-gold-font.py.
...
OUT = ROOT / 'public/generated-flyers/assets/png-glyphs/clean-gold'
...
for sheet_index, chars in enumerate(SHEETS, 1):
    path = SOURCE_DIR / f'clean-gold{sheet_index:02d}.png'
...
FONT.setupNameTable({'familyName':'Clean Gold PNG','styleName':'Regular','uniqueFontIdentifier':'CleanGoldPNG-1','fullName':'Clean Gold PNG','psName':'CleanGoldPNG','version':'Version 1.000'})
...
FONT.font.flavor='woff2';FONT.save(ROOT/'public/fonts/CleanGoldPNG.woff2')
(OUT/'metrics.json').write_text(json.dumps({'family':'Clean Gold PNG','version':1,'bitmapPpem':PPEM,'sources':SOURCES,'glyphs':REPORT,'kerningPairs':len(pairs)},indent=2)+'\n')
