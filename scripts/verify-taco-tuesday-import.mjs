...
try{
 ...
 for(const [id,file] of [['taco-tuesday','taco-tuesday.nflyer']]){
   ...
   if(!process.env.NF_ROUNDTRIP_ONLY) for(const format of ['square','story']){
     ...
     assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).replace(/\s/g,'').includes('TACO'));
     ...
     await page.waitForFunction((format)=>{
       ...
       return board?.textContent.replace(/\s/g,'').includes('TACO') && !/Preparing (?:story|square) canvas/.test(board.textContent) && rect && (format==='story'?rect.height/rect.width>1.7:Math.abs(rect.height/rect.width-1)<0.05);
     ...
     if(!process.env.NF_PREVIEW_ONLY)for(const objectId of ["presenter", "headline", "subtitle", "day", "month", "date", "genres", "price1", "price2", "price3", "item1", "item2", "item3", "description", "venue", "address", "motto"]){
       ...
       if(['headline','subtitle'].includes(objectId)){
         ...
         assert.equal(await menu.getByRole('button',{name:/Textured Gold Serif/,includeHidden:true}).count(),1,'Shared gold font available in font options');
         assert.equal(await controls.locator('[data-png-glyph-picker]').getByRole('button',{name:/Textured Gold Serif/,includeHidden:true}).count(),1,'Shared gold font available in PNG gallery');
         await picker.locator('button').first().evaluate(el=>el.click());
       }
 ...
 for(const format of ['Square','Story']){
   ...
   await page.screenshot({clip:await page.locator('#artboard').boundingBox(),path:`${out}/taco-tuesday-${format.toLowerCase()}-roundtrip.png`});
 }
 if(process.env.NF_EXPORT_CHECKS){
   ...
   for(const format of ['Square','Story']){
     ...
     await writeFile(`${out}/taco-tuesday-${format.toLowerCase()}-export.png`,Buffer.from(data));
