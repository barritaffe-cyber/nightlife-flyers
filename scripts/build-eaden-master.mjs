...
const native={presenter:'presenter',headline:'headline',subtitle:'headline2',date:'date',genres:'details',venue:'venue',address:'address'};
const companions={presents:['presents','details','details'],prefix:['prefix','details','details'],day:['weekday','date','date'],month:['month','date','date'],age:['ageInfo','details','details'],lineupLabel:['lineupLabel','details2','details2'],dj1:['dj1','details2','details2'],dj2:['dj2','details2','details2'],genre1:['genre1','details','details'],genre2:['genre2','details','details'],genre3:['genre3','details','details']};
const semanticRoles={...Object.fromEntries(Object.entries(native).map(([id,semanticRole])=>[id,{semanticRole}])),...Object.fromEntries(Object.entries(companions).map(([id,[semanticRole,panel,uiField]])=>[id,{semanticRole,text:`eaden_${id}`,panel,uiField,moveTarget:panel,mappedControls:true,editable:true}]))};
const fontMap={Gold:'Eaden Gold PNG',EadenSerif:'Didot',Sans:'LEMONMILK-Light',Script:'Dear Script (Demo_Font)',...Object.fromEntries(Object.keys(semanticRoles).map(id=>[id,id==='headline'?'Eaden Gold PNG':id==='subtitle'?'Dear Script (Demo_Font)':['presenter','date','day','month','genres','venue','lineupLabel'].includes(id)?'Didot':'LEMONMILK-Light']))};
export const cocoCssMasterAdapter={
 id:'eaden',masterPath:new URL('../public/generated-flyers/eaden-master.html',import.meta.url),outputPath:new URL('../public/generated-flyers/eaden.nflyer',import.meta.url),publicRoot:new URL('../public/',import.meta.url),editorTextScale:.5,requiredFonts:['Gold','EadenSerif','Sans','Script'],requiredRoles:Object.values(native),semanticRoles,fontMap,
 recipe:{id:'como-una-boa',name:'Eaden — The Garden Lounge',version:3,runtime:{compositionPattern:'center-poster-stack',styleId:'black-electric',palette:{bgFrom:'#070707',bgTo:'#171714',primary:'#f9f6eb',secondary:'#aaa69b',accent:'#dcc38e',neutral:'#edece5'},authority:{layout:{owner:'recipe-elements'},palette:{owner:'recipe-element-paint'},assets:{owner:'recipe-elements'},crop:{owner:'recipe-background-image'}},formats:{square:{canvas:{width:1080,height:1080}},story:{canvas:{width:1080,height:1920}}}}},eventBrief:{eventName:'Eaden',subtitle:'Night',date:'08'}
};
if(await access(new URL('../lib/template-data/eaden-saved-source.json',import.meta.url)).then(()=>true,()=>false))throw Error('Eaden has an accepted user save; preserve it instead of rebuilding.');
...
for(const v of [result.square,result.story]){
 Object.assign(v,{backgroundUrl:blank,bgUrl:blank,subtagEnabled:false,rightRailEnabled:false,rightRailLabel:'',presenterEnabled:true,venueEnabled:true,time:"",priceEnabled:false,priceLabel:'',qrEnabled:false,qrImageUrl:null,qrX:v===result.story?85:88.8,qrY:v===result.story?18:25.7,qrScale:7/11,leftRailEnabled:false,leftRail:'',leftRailLabel:'',headShadow:false,detailsShadow:false,venueShadow:false,socialHandleEnabled:false,cocoSocialHandleEnabled:false,details2Enabled:false,details2:'',djLineupLabel:'',exp:1,contrast:1/.9,saturation:1,warmth:0,tint:0,gamma:1,grain:0,vibrance:0,filmGrade:0});
...
await writeFile(new URL('../lib/template-data/eaden-v2.json',import.meta.url),JSON.stringify({square:result.square,story:result.story},null,2)+'\n');
