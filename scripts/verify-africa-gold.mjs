...
const glyphFamily='Africa Gold PNG';
const sampleText = 'Africa\nGold';
const out = process.env.NF_OUTPUT_DIR || '/tmp/africa-gold-check';
...
try{
 await page.goto(`${baseUrl}/generated-flyers/africa-gold-font-preview.html`);
 ...
 for(const [id,file] of [['glyph-check',process.env.NF_PROJECT || 'day-party.nflyer']]){
   ...
   for(const [objectId,panel,placeholder] of [['headline','headline','ENTER HEADLINE...'],['subtitle','head2','Optional sub-headline']]){
     ...
     for(const [id,panel,placeholder,text,size] of [
       ['headline','headline','ENTER HEADLINE...','Africa Gold','48'],
   ...
   assert.equal(errors.length,0,JSON.stringify(errors));console.log('AFRICA GOLD: BOTH PICKERS, FORMATS, EDITING, ROUNDTRIP, AND PNG EXPORT PASSED');
 }
