...
try{
 ...
 for(const [id,file] of [['aura','aura.nflyer']]){
   ...
   if(!process.env.NF_ROUNDTRIP_ONLY) for(const format of ['square','story']){
     ...
     assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).replace(/\s/g,'').includes('AURA'));
     assert.equal(await page.locator('[data-coco-compiled-object="subtitle"]').count(),1);
     const tracking=await page.locator('[data-coco-compiled-object="headline"]').evaluate(el=>parseFloat(getComputedStyle(el).letterSpacing)/parseFloat(getComputedStyle(el).fontSize));assert.ok(Math.abs(tracking+.39)<.001,'Requested headline spacing');
     ...
     await page.waitForFunction((format)=>{
       ...
       return board?.textContent.replace(/\s/g,'').includes('AURA') && !/Preparing (?:story|square) canvas/.test(board.textContent) && rect && (format==='story'?rect.height/rect.width>1.7:Math.abs(rect.height/rect.width-1)<0.05);
     ...
     if(!process.env.NF_PREVIEW_ONLY)for(const objectId of ["presenter", "headline", "subtitle", "day", "month", "date", "year", "genres", "description", "venue", "address", "handle", "entry", "entryLabel", "parking", "rsvp", "qrCaption"]){
 ...
 if(['genres','address'].includes(objectId)){
       ...
       if(objectId==='genres'){
         ...
         assert.ok(Math.abs(tracking-.4)<.001,format+' preserves authored details tracking: '+tracking);
 ...
 await page.waitForTimeout(2000);
 await page.locator('[aria-label="Preparing flyer canvas"]').waitFor({state:'hidden',timeout:90000});
 assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).includes('AURA'));
 ...
 for(const format of ['Square','Story']){
   ...
   await page.waitForFunction((format)=>{
     ...
     const ready=box && (format==='Story'?box.height/box.width>1.7:Math.abs(box.height/box.width-1)<.05) && Math.abs(tracking-.4)<.001;
   ...
   const restoredTracking=await page.locator('[data-coco-compiled-object="headline"]').evaluate(el=>parseFloat(getComputedStyle(el).letterSpacing)/parseFloat(getComputedStyle(el).fontSize));assert.ok(Math.abs(restoredTracking+.39)<.001,'Headline spacing survives save/reopen');
   await page.screenshot({clip:await page.locator('#artboard').boundingBox(),path:`${out}/aura-${format.toLowerCase()}-roundtrip.png`});
 }
 if(process.env.NF_EXPORT_CHECKS){
   ...
   for(const format of ['Square','Story']){
     ...
     await writeFile(`${out}/aura-${format.toLowerCase()}-export.png`,Buffer.from(data));
