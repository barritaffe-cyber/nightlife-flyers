...
const companions={"brand": ["brand", "details", "details"], "brandSub": ["brandSub", "details", "details"], "hours": ["hours", "date", "date"], "discount": ["discount", "details", "details"], "mood": ["mood", "details", "details"], "reservation": ["reservation", "details", "details"], "motto": ["motto", "details", "details"]};
const semanticRoles={...Object.fromEntries(Object.entries(native).map(([id,semanticRole])=>[id,{semanticRole}])),...Object.fromEntries(Object.entries(companions).map(([id,[semanticRole,panel,uiField]])=>[id,{semanticRole,text:id==='detailsLabel'?'detailsLabel':`branchHappyHour_${id}`,panel,uiField,moveTarget:panel,mappedControls:true,editable:true}]))};
const fontMap={TieSerif:'Didot',Script:'Dear Script (Demo_Font)',Sans:'LEMONMILK-Light',...Object.fromEntries(Object.keys(semanticRoles).map(id=>[id,['headline','discount'].includes(id)?'Didot':['subtitle','motto'].includes(id)?'Dear Script (Demo_Font)':'LEMONMILK-Light']))};
export const cocoCssMasterAdapter={
 id:'branch-happy-hour',masterPath:new URL('../public/generated-flyers/branch-happy-hour-master.html',import.meta.url),outputPath:new URL('../public/generated-flyers/branch-happy-hour.nflyer',import.meta.url),publicRoot:new URL('../public/',import.meta.url),editorTextScale:.5,requiredFonts:['TieSerif','Sans','Script'],requiredRoles:Object.values(native),semanticRoles,fontMap,
 recipe:{id:'branch-happy-hour',name:'Happy Hour — The Branch Rathskeller',version:2,runtime:{compositionPattern:'center-poster-stack',styleId:'black-electric',palette:{bgFrom:'#070707',bgTo:'#171714',primary:'#f9f6eb',secondary:'#aaa69b',accent:'#dcc38e',neutral:'#edece5'},authority:{layout:{owner:'recipe-elements'},palette:{owner:'recipe-element-paint'},assets:{owner:'recipe-elements'},crop:{owner:'recipe-background-image'}},formats:{square:{canvas:{width:1080,height:1080}},story:{canvas:{width:1080,height:1920}}}}},eventBrief:{eventName:'HAPPY',subtitle:'Hour',date:'MONDAY - FRIDAY'}
};
if(await access(new URL('../lib/template-data/branch-happy-hour-saved-source.json',import.meta.url)).then(()=>true,()=>false))throw Error('HAPPY has an accepted user save; preserve it instead of rebuilding.');
...
for(const v of [result.square,result.story]){
 Object.assign(v,{backgroundUrl:blank,bgUrl:blank,subtagEnabled:false,rightRailEnabled:false,rightRailLabel:'',presenterEnabled:true,priceEnabled:false,priceLabel:'',qrEnabled:false,qrImageUrl:null,qrX:v===result.story?85:88.8,qrY:v===result.story?18:25.7,qrScale:7/11,leftRailEnabled:false,leftRail:'',leftRailLabel:'',headShadow:false,detailsShadow:false,venueShadow:false,socialHandleEnabled:false,cocoSocialHandleEnabled:false,details2Enabled:false,details2:'',djLineupLabel:'',exp:1,contrast:1/.9,saturation:1,warmth:0,tint:0,gamma:1,grain:0,vibrance:0,filmGrade:0});
...
await writeFile(new URL('../lib/template-data/branch-happy-hour-v2.json',import.meta.url),JSON.stringify({square:result.square,story:result.story},null,2)+'\n');
