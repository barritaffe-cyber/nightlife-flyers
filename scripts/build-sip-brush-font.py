...
configs={'sip-brush':('paint-sip2', 'Sunset Paint Brush', 'SunsetPaintBrushPNG', [])}
...
if key=='sip-brush':
 ...
 cells=[('ABCDEFG',[15,271,490,688,892,1092,1276,1520],0,219,194,173),('HIJKLMN',[30,258,395,611,838,1050,1267,1515],219,440,410,185),('OPQRSTU',[25,264,475,674,886,1100,1300,1520],440,655,626,184),('VWXYZ',[195,402,639,853,1085,1365],655,847,825,166),('0123456789',[30,215,310,471,607,752,914,1060,1205,1340,1510],847,1024,991,136)]
 for chars,edges,top,bottom,baseline,cap in cells:
  for i,ch in enumerate(chars):
   owned=[c for c in components if edges[i]<=sum(x for x,y in c)/len(c)<edges[i+1] and top<=sum(y for x,y in c)/len(c)<bottom]
   assigned[ch]=set().union(*owned)
