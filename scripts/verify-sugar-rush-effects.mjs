...
const out = process.env.NF_OUTPUT_DIR || (process.env.NF_ACCEPTED_PREVIEW ? 'public/generated-flyers' : '/tmp/sugar-rush-effects');
...
try{
 ...
 for(const file of ['sugar-rush.nflyer']){
   ...
   console.log('READY');
   const lateChoice=page.getByRole('button',{name:'Choose Square format',exact:true});if(await lateChoice.isVisible()){await lateChoice.click();const keep=page.getByRole('button',{name:'Keep this layout',exact:true});if(await keep.isVisible())await keep.click();await page.locator('input[type=file][accept*=json]').setInputFiles('public/generated-flyers/sugar-rush.nflyer');await page.waitForTimeout(2000);}
