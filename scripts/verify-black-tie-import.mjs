...
try{
 ...
 for(const [id,file] of [['black-tie','black-tie.nflyer']]){
   ...
   for(const format of ['square','story']){
     ...
     assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).replace(/\s/g,'').includes('BLACK'));
     ...
     await page.waitForFunction((format)=>{
       ...
       return board?.textContent.replace(/\s/g,'').includes('BLACK') && !/Preparing (?:story|square) canvas/.test(board.textContent) && rect && (format==='story'?rect.height/rect.width>1.7:Math.abs(rect.height/rect.width-1)<0.05);
     ...
     if(!process.env.NF_PREVIEW_ONLY)for(const objectId of ["presenter","presents","headline","subtitle","tagline","details","day","month","date","time","venue","address","mood","invitation"]){
       ...
       if(objectId==='details'){
         ...
         assert.ok(Math.abs(tracking-.55)<.001,format+' preserves authored details tracking: '+tracking);
 ...
 const savedPath='/tmp/black-tie-roundtrip.nflyer';
 await download.saveAs(savedPath);
 await page.locator('input[type=file][accept*=json]').setInputFiles(savedPath);
 await page.waitForTimeout(2000);
 await page.locator('[aria-label="Preparing flyer canvas"]').waitFor({state:'hidden',timeout:90000});
 assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).includes('BLACK'));
 ...
 for(const format of ['Square','Story']){
   ...
   assert.ok(Math.abs(tracking-.55)<.001,format+' retains details tracking after reopening: '+tracking);
   await page.screenshot({clip:await page.locator('#artboard').boundingBox(),path:`${out}/black-tie-${format.toLowerCase()}-preview.png`});
 }
 if(process.env.NF_EXPORT_CHECKS){
   ...
   for(const format of ['Square','Story']){
     ...
     await writeFile(`${out}/black-tie-${format.toLowerCase()}-export.png`,Buffer.from(data));
