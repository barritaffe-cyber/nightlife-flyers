...
const companions={day:['weekday','date','date'],year:['year','date','date'],mood:['mood','details','details'],railText:['railText','details','details'],age:['ageInfo','details','details'],parking:['parking','details','details'],qrCaption:['qrCaption','details','details'],detailsLabel:['detailsLabel','details','detailsLabel']};
const semanticRoles={...Object.fromEntries(Object.entries(native).map(([id,semanticRole])=>[id,{semanticRole}])),...Object.fromEntries(Object.entries(companions).map(([id,[semanticRole,panel,uiField]])=>[id,{semanticRole,text:id==='detailsLabel'?'detailsLabel':`bassPressure_${id}`,panel,uiField,moveTarget:panel,mappedControls:true,editable:true}]))};
const fontMap={Blue:'Bass Pressure Blue PNG',Brush:'Drift Brush SVG',Condensed:'Bebas Neue',Sans:'LEMONMILK-Light',...Object.fromEntries(Object.keys(semanticRoles).map(id=>[id,id==='headline'?'Bass Pressure Blue PNG':id==='subtitle'?'Drift Brush SVG':['date','venue'].includes(id)?'Bebas Neue':'LEMONMILK-Light']))};
export const cocoCssMasterAdapter={
 id:'bass-pressure',masterPath:new URL('../public/generated-flyers/bass-pressure-master.html',import.meta.url),outputPath:new URL('../public/generated-flyers/bass-pressure.nflyer',import.meta.url),publicRoot:new URL('../public/',import.meta.url),editorTextScale:.5,requiredFonts:['Blue','Brush','Condensed','Sans'],requiredRoles:Object.values(native),semanticRoles,fontMap,
 recipe:{id:'bass-pressure',name:'Bass Pressure — Concrete',version:2,runtime:{compositionPattern:'center-poster-stack',styleId:'black-electric',palette:{bgFrom:'#070707',bgTo:'#171714',primary:'#f9f6eb',secondary:'#aaa69b',accent:'#dcc38e',neutral:'#edece5'},authority:{layout:{owner:'recipe-elements'},palette:{owner:'recipe-element-paint'},assets:{owner:'recipe-elements'},crop:{owner:'recipe-background-image'}},formats:{square:{canvas:{width:1080,height:1080}},story:{canvas:{width:1080,height:1920}}}}},eventBrief:{eventName:'BASS',subtitle:'PRESSURE',date:'DEC 1'}
};
if(await access(new URL('../lib/template-data/bass-pressure-saved-source.json',import.meta.url)).then(()=>true,()=>false))throw Error('Bass Pressure has an accepted user save; preserve it instead of rebuilding.');
...
for(const v of [result.square,result.story]){
 Object.assign(v,{backgroundUrl:blank,bgUrl:blank,subtagEnabled:false,rightRailEnabled:true,rightRailLabel:'',presenterEnabled:true,priceEnabled:false,priceLabel:'',qrEnabled:true,qrImageUrl:null,qrX:89,qrY:v===result.story?92:85,qrScale:6.5/11,leftRailEnabled:false,leftRail:'',leftRailLabel:'',headShadow:false,detailsShadow:false,venueShadow:false,socialHandleEnabled:false,cocoSocialHandleEnabled:false,details2Enabled:false,details2:'',djLineupLabel:'',exp:1,contrast:1/.9,saturation:1,warmth:0,tint:0,gamma:1,grain:0,vibrance:0,filmGrade:0});
 ...
 const bounds={x:20,y:v===result.story?83:73,width:45,height:2};
...
await writeFile(new URL('../lib/template-data/bass-pressure-v2.json',import.meta.url),JSON.stringify({square:result.square,story:result.story},null,2)+'\n');
