...
const out=process.env.NF_BAD_GIRLS_AUDIT_DIR || '/tmp/bad-girls';
const projectPath=process.env.NF_BAD_GIRLS_IMPORT || 'public/generated-flyers/bad-girls.nflyer';
...
if(process.env.NF_BAD_GIRLS_EXPORT_ACCESS_FIXTURE==='1') {
  ...
  for(const format of ['square','story']) {
    ...
    await page.waitForFunction(({format,headline})=>{
      ...
      const ready=b&&(format==='square'?Math.abs(b.width/b.height-1)<.01:b.height/b.width>1.7)
        &&root.querySelectorAll('[data-coco-text-value]').length>=25
      ...
      if(!ready){window.__badGirlsReadyAt=0;return false;}
      window.__badGirlsReadyAt ||= Date.now();
      return Date.now()-window.__badGirlsReadyAt>1000;
    ...
    assert.equal(objects.filter(o=>o.text).length,24);
    ...
    for(const id of ['headline','subtitle']) {
      const run=page.locator(`#artboard [data-coco-compiled-object="${id}"] [data-png-glyph-run]`).first();
      const scale=Number(await run.getAttribute('data-png-fit-scale'));
      assert.ok(scale>0 && scale<=1, `${format}/${id}: glyph width fitting loaded`);
      const override=expected.compiledObjectOverrides?.[id]?.text;
      if(override && ['NOVA','LUNA'].includes(override)) assert.ok(scale<.95, `${format}/${id}: longer replacement fits the saved title lane`);
    }
    ...
    if(process.env.NF_BAD_GIRLS_PUBLISH_PREVIEWS==='1')await page.locator('#artboard').screenshot({path:`public/generated-flyers/bad-girls-${format}-preview.png`});
    await captureStyle.evaluate(node=>node.remove());
    console.log('PASS',format,objects.length,'text owners');
  }
  if(process.env.NF_BAD_GIRLS_EXPORT_CHECK==='1') {
    ...
    for(const format of ['square','story']) {
      ...
      await page.waitForFunction(format=>{
        const root=document.querySelector('#artboard'),b=root?.getBoundingClientRect();
        return b&&(format==='square'?Math.abs(b.width/b.height-1)<.01:b.height/b.width>1.7)&&root.querySelectorAll('[data-coco-text-value]').length>=25;
