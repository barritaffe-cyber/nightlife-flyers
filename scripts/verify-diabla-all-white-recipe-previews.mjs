...
try{
 ...
 for(const [id,file] of [['diabla-all-white','diabla-all-white-updated.nflyer']]){
   ...
   for(const format of ['square','story']){
     ...
     assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).includes('DIABLA'));
     ...
     await page.waitForFunction((format)=>{
       ...
       return board?.textContent.includes('DIABLA') && !/Preparing (?:story|square) canvas/.test(board.textContent) && rect && (format==='story'?rect.height/rect.width>1.7:Math.abs(rect.height/rect.width-1)<0.05);
     ...
     if(!process.env.NF_PREVIEW_ONLY)for(const objectId of ["weekday", "month", "date", "genres", "mood", "dress", "headline", "script", "tagline", "footer"]){
