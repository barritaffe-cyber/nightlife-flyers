...
const native={headline:'headline',subtitle:'headline2',date:'date',genres:'details'};
const companions={"styles": ["styles", "details", "details"], "day": ["day", "date", "date"], "hours": ["hours", "date", "date"], "footer": ["footer", "details", "details"]};
const semanticRoles={...Object.fromEntries(Object.entries(native).map(([id,semanticRole])=>[id,{semanticRole}])),...Object.fromEntries(Object.entries(companions).map(([id,[semanticRole,panel,uiField]])=>[id,{semanticRole,text:id==='detailsLabel'?'detailsLabel':`oneLove_${id}`,panel,uiField,moveTarget:panel,mappedControls:true,editable:true}]))};
const fontMap={Brush:'Drift Brush SVG',Sans:'LEMONMILK-Light',Bold:'LEMONMILK-Bold',...Object.fromEntries(Object.keys(semanticRoles).map(id=>[id,['headline','subtitle'].includes(id)?'Drift Brush SVG':id==='date'?'LEMONMILK-Bold':'LEMONMILK-Light']))};
export const cocoCssMasterAdapter={
 id:'one-love-reggae',masterPath:new URL('../public/generated-flyers/one-love-reggae-master.html',import.meta.url),outputPath:new URL('../public/generated-flyers/one-love-reggae.nflyer',import.meta.url),publicRoot:new URL('../public/',import.meta.url),editorTextScale:.5,requiredFonts:['Brush','Sans','Bold'],requiredRoles:Object.values(native),semanticRoles,fontMap,
 recipe:{id:'one-love-reggae',name:'One Love — Reggae Night',version:2,runtime:{compositionPattern:'center-poster-stack',styleId:'black-electric',palette:{bgFrom:'#070707',bgTo:'#171714',primary:'#f9f6eb',secondary:'#aaa69b',accent:'#dcc38e',neutral:'#edece5'},authority:{layout:{owner:'recipe-elements'},palette:{owner:'recipe-element-paint'},assets:{owner:'recipe-elements'},crop:{owner:'recipe-background-image'}},formats:{square:{canvas:{width:1080,height:1080}},story:{canvas:{width:1080,height:1920}}}}},eventBrief:{eventName:'ONE',subtitle:'LOVE',date:'APR 20'}
};
if(await access(new URL('../lib/template-data/one-love-reggae-saved-source.json',import.meta.url)).then(()=>true,()=>false))throw Error('HAPPY has an accepted user save; preserve it instead of rebuilding.');
...
for(const v of [result.square,result.story]){
 Object.assign(v,{backgroundUrl:blank,bgUrl:blank,subtagEnabled:false,rightRailEnabled:false,rightRailLabel:'',presenterEnabled:false,venueEnabled:false,venue:"",venueAddress:"",time:"",priceEnabled:false,priceLabel:'',qrEnabled:false,qrImageUrl:null,qrX:v===result.story?85:88.8,qrY:v===result.story?18:25.7,qrScale:7/11,leftRailEnabled:false,leftRail:'',leftRailLabel:'',headShadow:false,detailsShadow:false,venueShadow:false,socialHandleEnabled:false,cocoSocialHandleEnabled:false,details2Enabled:false,details2:'',djLineupLabel:'',exp:1,contrast:1/.9,saturation:1,warmth:0,tint:0,gamma:1,grain:0,vibrance:0,filmGrade:0});
...
await writeFile(new URL('../lib/template-data/one-love-reggae-v2.json',import.meta.url),JSON.stringify({square:result.square,story:result.story},null,2)+'\n');
