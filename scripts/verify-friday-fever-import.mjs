...
try{
 ...
 for(const [id,file] of [['friday-fever','friday-fever.nflyer']]){
   ...
   if(!process.env.NF_ROUNDTRIP_ONLY) for(const format of ['square','story']){
     ...
     assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).replace(/\s/g,'').includes('FRIDAY'));
     ...
     await page.waitForFunction((format)=>{
       ...
       return board?.textContent.replace(/\s/g,'').includes('FRIDAY') && !/Preparing (?:story|square) canvas/.test(board.textContent) && rect && (format==='story'?rect.height/rect.width>1.7:Math.abs(rect.height/rect.width-1)<0.05);
     ...
     if(!process.env.NF_PREVIEW_ONLY)for(const objectId of ["headline", "subtitle", "presenter", "presents", "genres", "venue", "address", "day", "date", "month", "sounds", "lineup", "dj2", "dj3", "footer"]){
 ...
 if(['genres','address'].includes(objectId)){
       ...
       if(objectId==='genres'){
         ...
         assert.ok(Math.abs(tracking-.6)<.001,format+' preserves authored details tracking: '+tracking);
 ...
 await page.waitForTimeout(2000);
 await page.locator('[aria-label="Preparing flyer canvas"]').waitFor({state:'hidden',timeout:90000});
 assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).includes('FRIDAY'));
 ...
 for(const format of ['Square','Story']){
   ...
   await page.waitForFunction((format)=>{
     ...
     const ready=box && (format==='Story'?box.height/box.width>1.7:Math.abs(box.height/box.width-1)<.05) && Math.abs(tracking-.6)<.001;
   ...
   await page.screenshot({clip:await page.locator('#artboard').boundingBox(),path:`${out}/friday-fever-${format.toLowerCase()}-roundtrip.png`});
 }
 if(process.env.NF_EXPORT_CHECKS){
   ...
   for(const format of ['Square','Story']){
     ...
     await writeFile(`${out}/friday-fever-${format.toLowerCase()}-export.png`,Buffer.from(data));
