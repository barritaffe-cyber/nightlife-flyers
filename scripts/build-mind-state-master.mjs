...
const companions={day:['weekday','date','date'],year:['year','date','date'],lineup:['lineup','details','details'],handle:['handle','details','details'],age:['age','details','details'],parking:['parking','details','details'],qrCaption:['qrCaption','details','details'],detailsLabel:['detailsLabel','details','detailsLabel']};
const semanticRoles={...Object.fromEntries(Object.entries(native).map(([id,semanticRole])=>[id,{semanticRole}])),...Object.fromEntries(Object.entries(companions).map(([id,[semanticRole,panel,uiField]])=>[id,{semanticRole,text:id==='detailsLabel'?'detailsLabel':`mindState_${id}`,panel,uiField,moveTarget:panel,mappedControls:true,editable:true}]))};
const fontMap={Arctic:'Arctic Metal PNG',Tech:'Azonix',Sans:'LEMONMILK-Light',Tall:'Bebas Neue',...Object.fromEntries(Object.keys(semanticRoles).map(id=>[id,id==='headline'?'Arctic Metal PNG':['subtitle','lineup'].includes(id)?'Azonix':['day','date','venue','age','qrCaption'].includes(id)?'Bebas Neue':'LEMONMILK-Light']))};
export const cocoCssMasterAdapter={
 id:'mind-state',masterPath:new URL('../public/generated-flyers/mind-state-master.html',import.meta.url),outputPath:new URL('../public/generated-flyers/mind-state.nflyer',import.meta.url),publicRoot:new URL('../public/',import.meta.url),editorTextScale:.5,requiredFonts:['Arctic','Tech','Sans','Tall'],requiredRoles:Object.values(native),semanticRoles,fontMap,
 recipe:{id:'mind-state',name:'Mind State — Arctic Metal',version:2,runtime:{compositionPattern:'center-poster-stack',styleId:'black-electric',palette:{bgFrom:'#070707',bgTo:'#171714',primary:'#f9f6eb',secondary:'#aaa69b',accent:'#dcc38e',neutral:'#edece5'},authority:{layout:{owner:'recipe-elements'},palette:{owner:'recipe-element-paint'},assets:{owner:'recipe-elements'},crop:{owner:'recipe-background-image'}},formats:{square:{canvas:{width:1080,height:1080}},story:{canvas:{width:1080,height:1920}}}}},eventBrief:{eventName:'MIND\nSTATE',subtitle:'NEURAL // CORE',date:'DEC 01 2026'}
};
if(await access(new URL('../lib/template-data/mind-state-saved-source.json',import.meta.url)).then(()=>true,()=>false))throw Error('Mind State has an accepted user save; preserve it instead of rebuilding.');
...
for(const v of [result.square,result.story]){
 Object.assign(v,{backgroundUrl:blank,bgUrl:blank,subtagEnabled:false,rightRailEnabled:true,rightRailLabel:'',presenterEnabled:true,priceEnabled:false,priceLabel:'',qrEnabled:true,qrImageUrl:null,qrX:89,qrY:v===result.story?12:15.2,qrScale:6.4/11,leftRailEnabled:false,leftRail:'',leftRailLabel:'',headShadow:false,socialHandleEnabled:false,cocoSocialHandleEnabled:false,details2Enabled:false,details2:'',djLineupLabel:'',exp:1,contrast:1/.9,saturation:1,warmth:0,tint:0,gamma:1,grain:0,vibrance:0,filmGrade:0});
 ...
 const bounds={x:4,y:v===result.story?22:29,width:30,height:2};
...
await writeFile(new URL('../lib/template-data/mind-state-v2.json',import.meta.url),JSON.stringify({square:result.square,story:result.story},null,2)+'\n');
