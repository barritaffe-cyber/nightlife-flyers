...
try{
 ...
 for(const [id,file] of [['miami-ocean-nights','miami-ocean-nights.nflyer']]){
   ...
   if(!process.env.NF_ROUNDTRIP_ONLY) for(const format of ['square','story']){
     ...
     if(!process.env.NF_PREVIEW_ONLY)for(const objectId of ["presenter", "headline", "subtitle", "mood", "day", "date", "hours", "genres", "description", "venue", "address", "offer", "age", "dress", "qrCaption"]){
 ...
 for(const format of ['Square','Story']){
   ...
   await page.screenshot({clip:await page.locator('#artboard').boundingBox(),path:`${out}/miami-ocean-nights-${format.toLowerCase()}-roundtrip.png`});
 }
 if(process.env.NF_EXPORT_CHECKS){
   ...
   for(const format of ['Square','Story']){
     ...
     await writeFile(`${out}/miami-ocean-nights-${format.toLowerCase()}-export.png`,Buffer.from(data));
