 ...
 for(const [id,file] of [['branch-happy-hour','branch-happy-hour.nflyer']]){
   ...
   if(!process.env.NF_ROUNDTRIP_ONLY) for(const format of ['square','story']){
     ...
     for(const [id,panel] of [['headline','headline'],['subtitle','head2']]) {
       ...
       for(const [label,align] of [['C','center'],['R','right'],['L','left']]) {
         await controls.getByRole('button',{name:label,exact:true}).click();
         await page.waitForTimeout(400);
         assert.equal(await owner.getAttribute('data-coco-compiled-align'),align,id+' '+format);
         assert.equal(await owner.evaluate(el=>getComputedStyle(el).textAlign),align);
       }
