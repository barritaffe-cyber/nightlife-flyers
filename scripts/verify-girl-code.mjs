...
// Full run: NF_GIRL_CODE_EXPORT_ACCESS_FIXTURE=1 node scripts/verify-girl-code.mjs
// Artwork iteration: NF_GIRL_CODE_CONTROLS_CHECK=0 NF_GIRL_CODE_EXPORT_CHECK=0 ...
// Use NF_GIRL_CODE_FORMATS=square or story for a focused artwork iteration.
const out = process.env.NF_GIRL_CODE_AUDIT_DIR || '/tmp/girl-code';
const projectPath = process.env.NF_GIRL_CODE_IMPORT || 'public/generated-flyers/girl-code.nflyer';
const baseUrl = process.env.NF_BASE_URL || 'http://localhost:3000';
const runControls = process.env.NF_GIRL_CODE_CONTROLS_CHECK !== '0';
const runExports = process.env.NF_GIRL_CODE_EXPORT_CHECK !== '0';
const runOriginalExports = runExports && process.env.NF_GIRL_CODE_ORIGINAL_EXPORT_CHECK !== '0';
const formats = (process.env.NF_GIRL_CODE_FORMATS || 'square,story').split(',');
...
const initialIds = ['headline','subtitle','mood','motto','day','month','date','time','endTime','tagline','djLabel','dj1','dj2','dj3','genres','venue','venueCaption','address','contactLabel','contact'];
...
if (process.env.NF_GIRL_CODE_EXPORT_ACCESS_FIXTURE === '1') {
...
async function capture(name, publishFormat) {
  ...
  if (publishFormat && process.env.NF_GIRL_CODE_PUBLISH_PREVIEWS === '1') {
    await board.screenshot({path: `public/generated-flyers/girl-code-${publishFormat}-preview.png`, animations: 'disabled'});
  }
...
try {
  ...
  for (const format of formats) {
    ...
    assert.doesNotMatch(font, /PNG/, 'GIRL uses editable live type with texture');
  ...
  if (runControls) {
    ...
    for (const format of formats) {
      await ready(format, initialMaps[format]);
      const checkObjects=process.env.NF_GIRL_CODE_CHECK_OBJECTS?.split(',');
