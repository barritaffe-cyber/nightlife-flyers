...
try{
 ...
 for(const [id,file] of [['reggae-jams','reggae-jams-updated.nflyer']]){
   ...
   for(const format of ['square','story']){
     ...
     await page.waitForFunction((format)=>{
       ...
       return board?.textContent.includes('REGGAE') && !/Preparing (?:story|square) canvas/.test(board.textContent) && rect && (format==='story'?rect.height/rect.width>1.7:Math.abs(rect.height/rect.width-1)<0.05);
     },format,{timeout:90000});
     await page.evaluate(()=>document.fonts.ready);
     await page.waitForTimeout(4000);
     await page.locator('#artboard').screenshot({path:`${out}/${id}-${format}-v2-subject-b352c062.png`});
