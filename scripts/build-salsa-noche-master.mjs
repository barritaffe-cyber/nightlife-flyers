...
const companions={"day": ["day", "date", "date"], "hours": ["hours", "date", "date"], "mood": ["mood", "details", "details"], "dress": ["dress", "details", "details"], "tagline": ["tagline", "details", "details"], "lessons": ["lessons", "details", "details"], "doors": ["doors", "details", "details"], "footer": ["footer", "details", "details"], "qrCaption": ["qrCaption", "details", "details"]};
const semanticRoles={...Object.fromEntries(Object.entries(native).map(([id,semanticRole])=>[id,{semanticRole}])),...Object.fromEntries(Object.entries(companions).map(([id,[semanticRole,panel,uiField]])=>[id,{semanticRole,text:id==='detailsLabel'?'detailsLabel':`salsaNoche_${id}`,panel,uiField,moveTarget:panel,mappedControls:true,editable:true}]))};
const fontMap={TieSerif:'Didot',Script:'Dear Script (Demo_Font)',Condensed:'Bebas Neue',Sans:'LEMONMILK-Light',...Object.fromEntries(Object.keys(semanticRoles).map(id=>[id,id==='headline'?'Dear Script (Demo_Font)':['subtitle','date'].includes(id)?'Didot':['day','lessons','doors'].includes(id)?'Bebas Neue':'LEMONMILK-Light']))};
export const cocoCssMasterAdapter={
 id:'salsa-noche',masterPath:new URL('../public/generated-flyers/salsa-noche-master.html',import.meta.url),outputPath:new URL('../public/generated-flyers/salsa-noche.nflyer',import.meta.url),publicRoot:new URL('../public/',import.meta.url),editorTextScale:.5,requiredFonts:['TieSerif','Sans','Script','Condensed'],requiredRoles:Object.values(native),semanticRoles,fontMap,
 recipe:{id:'salsa-noche',name:'Salsa Noche — La Fiesta',version:2,runtime:{compositionPattern:'center-poster-stack',styleId:'black-electric',palette:{bgFrom:'#070707',bgTo:'#171714',primary:'#f9f6eb',secondary:'#aaa69b',accent:'#dcc38e',neutral:'#edece5'},authority:{layout:{owner:'recipe-elements'},palette:{owner:'recipe-element-paint'},assets:{owner:'recipe-elements'},crop:{owner:'recipe-background-image'}},formats:{square:{canvas:{width:1080,height:1080}},story:{canvas:{width:1080,height:1920}}}}},eventBrief:{eventName:'Salsa',subtitle:'NOCHE',date:'DEC 1'}
};
if(await access(new URL('../lib/template-data/salsa-noche-saved-source.json',import.meta.url)).then(()=>true,()=>false))throw Error('Salsa Noche has an accepted user save; preserve it instead of rebuilding.');
...
for(const v of [result.square,result.story]){
 Object.assign(v,{backgroundUrl:blank,bgUrl:blank,subtagEnabled:false,rightRailEnabled:false,rightRailLabel:'',presenterEnabled:true,priceEnabled:false,priceLabel:'',qrEnabled:true,qrImageUrl:null,qrX:v===result.story?84:90,qrY:v===result.story?89:58,qrScale:6.7/11,leftRailEnabled:false,leftRail:'',leftRailLabel:'',headShadow:false,detailsShadow:false,venueShadow:false,socialHandleEnabled:false,cocoSocialHandleEnabled:false,details2Enabled:false,details2:'',djLineupLabel:'',exp:1,contrast:1/.9,saturation:1,warmth:0,tint:0,gamma:1,grain:0,vibrance:0,filmGrade:0});
...
await writeFile(new URL('../lib/template-data/salsa-noche-v2.json',import.meta.url),JSON.stringify({square:result.square,story:result.story},null,2)+'\n');
