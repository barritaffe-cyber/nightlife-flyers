...
const out = process.env.NF_OUTPUT_DIR || '/tmp/girl-code-discovery';
const eventName = process.env.NF_EVENT_NAME || 'Girl Code';
const theme = process.env.NF_THEME || 'Tropical';
const recipeId = process.env.NF_RECIPE_ID || 'girl-code';
...
try {
  ...
  console.log('PASS Girl Code / Tropical through both personalized previews to the details form.');
} catch (error) {
