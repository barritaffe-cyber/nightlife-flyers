...
// Full run: NF_SOFT_LIFE_EXPORT_ACCESS_FIXTURE=1 node scripts/verify-soft-life.mjs
// Artwork iteration: NF_SOFT_LIFE_CONTROLS_CHECK=0 NF_SOFT_LIFE_EXPORT_CHECK=0 ...
// Use NF_SOFT_LIFE_FORMATS=square or story for a focused artwork iteration.
const out = process.env.NF_SOFT_LIFE_AUDIT_DIR || '/tmp/soft-life';
...
const runControls = process.env.NF_SOFT_LIFE_CONTROLS_CHECK !== '0';
const runExports = process.env.NF_SOFT_LIFE_EXPORT_CHECK !== '0';
const runOriginalExports = runExports && process.env.NF_SOFT_LIFE_ORIGINAL_EXPORT_CHECK !== '0';
...
if (process.env.NF_SOFT_LIFE_EXPORT_ACCESS_FIXTURE === '1') {
...
async function capture(name, publishFormat) {
  ...
  if (publishFormat && process.env.NF_SOFT_LIFE_PUBLISH_PREVIEWS === '1') {
    await board.screenshot({path: `public/generated-flyers/soft-life-${publishFormat}-preview.png`, animations: 'disabled'});
  }
...
try {
  ...
  for (const format of formats) {
    ...
    assert.doesNotMatch(font, /PNG/, 'SOFT uses editable live type with texture');
  ...
  if (runControls) {
    ...
    for (const format of formats) {
      ...
      for (const id of initialIds) {
        ...
        if (['presents', 'djLabel', 'dressLabel'].includes(id)) {
