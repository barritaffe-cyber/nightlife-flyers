...
const out = process.env.NF_OUTPUT_DIR || (process.env.NF_ACCEPTED_PREVIEW ? 'public/generated-flyers' : '/tmp/drift-kingz-effects');
...
try{
 ...
 for(const file of ['drift-kingz.nflyer']){
   ...
   console.log('READY');
   const lateChoice=page.getByRole('button',{name:'Choose Square format',exact:true});if(await lateChoice.isVisible()){await lateChoice.click();const keep=page.getByRole('button',{name:'Keep this layout',exact:true});if(await keep.isVisible())await keep.click();await page.locator('input[type=file][accept*=json]').setInputFiles('public/generated-flyers/drift-kingz.nflyer');await page.waitForTimeout(2000);}
   ...
   for(const format of ['Square','Story']){
     ...
     for(const [objectId,panel] of [['headline','headline'],['subtitle','head2']]){
       ...
       const paint=()=>owner.evaluate(el=>Array.from(el.querySelectorAll('span')).map(n=>{const s=getComputedStyle(n);return s.filter+'|'+s.textShadow}).join(';'));
       ...
       if(objectId==='subtitle'){const spacing=controls.locator('label').filter({hasText:/^Spacing$/}).locator('..').locator('input[type=text]').first();await spacing.fill(format==='Square'?'-0.39':'-0.6');await spacing.press('Enter');}
 ...
 for(const format of ['square','story']){assert.equal(data.state.session[format].headShadow,format==='square');assert.equal(data.state.session[format].head2Shadow,format==='square');assert.equal(data.state.session[format].head2Fx.tracking,format==='square'?-.39:-.6);}
 ...
 for(const format of ['Square','Story','Square']){
   ...
   for(const [id,panel] of [['headline','headline'],['subtitle','head2']]){
     ...
     if(id==='subtitle'){const tracking=await page.locator('[data-coco-compiled-object="subtitle"]').evaluate(el=>parseFloat(getComputedStyle(el).letterSpacing)/parseFloat(getComputedStyle(el).fontSize));assert.ok(Math.abs(tracking-(format==='Square'?-.39:-.6))<.001,'Subtitle spacing persists independently');}
