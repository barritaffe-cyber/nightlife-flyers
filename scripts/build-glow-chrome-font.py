"""Extract supplied Glow Chrome sheets and package editable UI bitmap lettering.

Run with /tmp/offshore-font-build/bin/python scripts/build-glow-chrome-font.py.
...
OUT = ROOT / 'public/generated-flyers/assets/png-glyphs/glow-chrome'
...
for sheet_index, chars in enumerate(SHEETS, 1):
    path = SOURCE_DIR / f'glow-neon{sheet_index:02d}.png'
...
FONT.setupNameTable({'familyName':'Glow Chrome PNG','styleName':'Regular','uniqueFontIdentifier':'GlowChromePNG-2','fullName':'Glow Chrome PNG','psName':'GlowChromePNG','version':'Version 2.000'})
...
FONT.font.flavor='woff2';FONT.save(ROOT/'public/fonts/GlowChromePNG.woff2')
(OUT/'metrics.json').write_text(json.dumps({'family':'Glow Chrome PNG','version':2,'bitmapPpem':PPEM,'sources':SOURCES,'glyphs':REPORT,'kerningPairs':len(pairs)},indent=2)+'\n')
