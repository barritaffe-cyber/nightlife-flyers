...
const out=process.env.NF_GLOW_AUDIT_DIR || '/tmp/glow';
const projectPath=process.env.NF_GLOW_IMPORT || 'public/generated-flyers/glow.nflyer';
...
page.on('request',r=>{if(r.url().includes('GlowChromePNG'))fontRequests.push(r.url())});
if(process.env.NF_GLOW_EXPORT_ACCESS_FIXTURE==='1') {
...
try {
  await page.goto('http://localhost:3000/generated-flyers/glow-font-specimens.html');
  await page.evaluate(()=>document.fonts.ready);
  await page.locator('section.chrome').screenshot({path:`${out}/specimen.png`});
  ...
  for(const format of ['square','story']) {
    ...
    await page.waitForFunction(({format,headline})=>{
      ...
      const ready=b&&(format==='square'?Math.abs(b.width/b.height-1)<.01:b.height/b.width>1.7)
        &&root.querySelectorAll('[data-coco-text-value]').length>=17
      ...
      if(!ready){window.__glowReadyAt=0;return false;}
      window.__glowReadyAt ||= Date.now();
      return Date.now()-window.__glowReadyAt>1000;
    ...
    assert.equal(objects.filter(o=>o.text).length,16);
    ...
    assert.ok(objects.find(o=>o.id==='headline').font.includes('Glow Chrome PNG'),'headline uses the new PNG family');
    assert.ok(await page.evaluate(()=>document.fonts.check('20px "Glow Chrome PNG"')),'glyph font is loaded');
    ...
    if(process.env.NF_GLOW_PUBLISH_PREVIEWS==='1')await page.locator('#artboard').screenshot({path:`public/generated-flyers/glow-${format}-preview.png`});
    await captureStyle.evaluate(node=>node.remove());
    console.log('PASS',format,objects.length,'text owners');
  }
  if(process.env.NF_GLOW_EXPORT_CHECK==='1') {
    ...
    for(const format of ['square','story']) {
      ...
      await page.waitForFunction(format=>{
        const root=document.querySelector('#artboard'),b=root?.getBoundingClientRect();
        return b&&(format==='square'?Math.abs(b.width/b.height-1)<.01:b.height/b.width>1.7)&&root.querySelectorAll('[data-coco-text-value]').length>=17;
  ...
  assert.ok(fontRequests.some(url=>url.includes('GlowChromePNG.woff2?v=2')),'editor/export use refreshed glyph font');
