...
const mobile=process.env.NF_DEVICE==='mobile', out=process.env.NF_OUTPUT_DIR || (mobile?'/tmp/coco-added-story-mobile':'/tmp/coco-added-story');mkdirSync(out,{recursive:true});
...
try {
 ...
 if(!mobile){
  const controls=page.getByTestId('coco-added-text-controls').filter({visible:true});
  assert.equal(await page.locator('#details-panel').getByText('Event Details',{exact:true}).count(),0,'new object never opens Event Details');
  assert.equal(await input.evaluate(el=>document.activeElement===el),true,'new object input receives focus');
  await controls.locator('input[type="text"]').first().fill('30');await controls.locator('input[type="text"]').first().press('Tab');
  await controls.getByRole('button',{name:'Font',exact:true}).click();
  const fonts=page.locator('[aria-label="Font choices"]');
  await fonts.getByRole('button').filter({hasText:'Club / Poster Headlines'}).click();
  await fonts.getByRole('button',{name:'Bebas Neue',exact:true}).click();
  ...
  const style=await added.first().evaluate(el=>({size:getComputedStyle(el).fontSize,color:getComputedStyle(el).color,align:getComputedStyle(el).textAlign}));
  console.log('Added text style',style);assert.equal(style.size,'30px');assert.equal(style.color,'rgb(32, 224, 238)');assert.equal(style.align,'left');
  await controls.locator('[data-added-control="leading"] input[type="text"]').fill('1.5');await controls.locator('[data-added-control="leading"] input[type="text"]').press('Tab');
  assert.equal(await added.first().evaluate(el=>getComputedStyle(el).lineHeight),'45px','line spacing uses the entered multiplier');
 }
 ...
 if(!mobile){
  await added.first().click({position:{x:8,y:8}});
