...
const out = process.env.NF_OUTPUT_DIR || (process.env.NF_ACCEPTED_PREVIEW ? 'public/generated-flyers' : '/tmp/png-text-shadow');
...
try{
 ...
 for(const file of [process.env.NF_VERIFY_FILE || 'eaden.nflyer']){
   ...
   console.log('READY');
   const lateChoice=page.getByRole('button',{name:'Choose Square format',exact:true});if(await lateChoice.isVisible()){await lateChoice.click();const keep=page.getByRole('button',{name:'Keep this layout',exact:true});if(await keep.isVisible())await keep.click();await page.locator('input[type=file][accept*=json]').setInputFiles('public/generated-flyers/eaden.nflyer');await page.waitForTimeout(2000);}
