...
try{
 ...
 for(const [id,file] of [['yacht-escape','yacht-escape.nflyer']]){
   ...
   for(const format of ['square','story']){
     ...
     assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).replace(/\s/g,'').includes('YACHT'));
     ...
     await page.waitForFunction((format)=>{
       ...
       return board?.textContent.replace(/\s/g,'').includes('YACHT') && !/Preparing (?:story|square) canvas/.test(board.textContent) && rect && (format==='story'?rect.height/rect.width>1.7:Math.abs(rect.height/rect.width-1)<0.05);
     ...
     if(!process.env.NF_PREVIEW_ONLY)for(const objectId of ["presents", "day", "month", "mood", "genres", "boardingLabel", "boarding", "sailingLabel", "sailing", "aside", "club", "views", "headline", "script", "date", "venue", "marina", "footer", "compliance"]){
