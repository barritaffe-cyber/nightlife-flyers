...
import argparse
parser=argparse.ArgumentParser();parser.add_argument("collection",choices=["eaden-gold"]);parser.parse_args()
...
OUT = ROOT / 'public/generated-flyers/assets/png-glyphs/eaden-gold'
...
SOURCES = []
LAYOUT = json.loads((ROOT/'scripts/eaden-glyph-layout-v1.json').read_text())
...
for sheet_index, chars in enumerate(SHEETS, 1):
    path = SOURCE_DIR / f'gold-eden{sheet_index:02d}.png'
    ...
    for index, char in enumerate(chars):
        ...
        # Keep the existing family's cap/x heights and baselines, retaining
        # each new glyph's aspect ratio rather than stretching it into a cell.
        old = LAYOUT[char]
        ink_height = bounds[3]-bounds[1]
        scale = old['inkHeight']*PIXEL_SCALE/ink_height
        ...
        advance = max(150, round(old['advance']+((bounds[2]-bounds[0])*scale-old['inkWidth']*PIXEL_SCALE)*UNITS_PER_PIXEL))
        name = f'uni{ord(char):04X}'
        CMAP[ord(char)] = name
        ox = round(5*PIXEL_SCALE-(bounds[0]-l+pad)*scale)
        oy = round(old['inkBottom']*PIXEL_SCALE-(b-bounds[3]+pad)*scale)
        ...
        baseline = bounds[3]-(80*PIXEL_SCALE-old['inkBottom']*PIXEL_SCALE)/scale
        profile = {}
        cp = core.load()
...
FONT.setupNameTable({'familyName':'Eaden Gold PNG','styleName':'Regular','uniqueFontIdentifier':'EadenGoldPNG-2','fullName':'Eaden Gold PNG','psName':'EadenGoldPNG','version':'Version 2.000'})
...
FONT.font.flavor='woff2';FONT.save(ROOT/'public/fonts/EadenGoldPNG.woff2')
(OUT/'metrics.json').write_text(json.dumps({'family':'Eaden Gold PNG','version':2,'bitmapPpem':PPEM,'sources':SOURCES,'glyphs':REPORT,'kerningPairs':len(pairs)},indent=2)+'\n')
