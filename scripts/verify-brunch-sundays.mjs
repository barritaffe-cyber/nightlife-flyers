...
// Full run: NF_BRUNCH_SUNDAYS_EXPORT_ACCESS_FIXTURE=1 node scripts/verify-brunch-sundays.mjs
// Artwork iteration: NF_BRUNCH_SUNDAYS_CONTROLS_CHECK=0 NF_BRUNCH_SUNDAYS_EXPORT_CHECK=0 ...
// Use NF_BRUNCH_SUNDAYS_FORMATS=square or story for a focused artwork iteration.
const out = process.env.NF_BRUNCH_SUNDAYS_AUDIT_DIR || '/tmp/brunch-sundays';
const projectPath = process.env.NF_BRUNCH_SUNDAYS_IMPORT || 'public/generated-flyers/brunch-sundays.nflyer';
const baseUrl = process.env.NF_BASE_URL || 'http://localhost:3000';
const runControls = process.env.NF_BRUNCH_SUNDAYS_CONTROLS_CHECK !== '0';
const runExports = process.env.NF_BRUNCH_SUNDAYS_EXPORT_CHECK !== '0';
const runOriginalExports = runExports && process.env.NF_BRUNCH_SUNDAYS_ORIGINAL_EXPORT_CHECK !== '0';
const formats = (process.env.NF_BRUNCH_SUNDAYS_FORMATS || 'square,story').split(',');
...
const initialIds = ['presenter','headline','subtitle','day','month','date','mood','motto','tagline','mug','dj1','dj2','dj3','offer','included','drinks','venue','venueCaption','address','contact','age'];
...
if (process.env.NF_BRUNCH_SUNDAYS_EXPORT_ACCESS_FIXTURE === '1') {
...
async function ready(format, expectedMap) {
  ...
  await page.waitForFunction(({format, expectedMap}) => {
    ...
    if (!signature || window.__brunchSundaysReadySignature !== signature) {
      window.__brunchSundaysReadySignature = signature;
      window.__brunchSundaysReadyAt = performance.now();
      return false;
    }
    return performance.now() - window.__brunchSundaysReadyAt > 1800;
...
async function capture(name, publishFormat) {
  ...
  if (publishFormat && process.env.NF_BRUNCH_SUNDAYS_PUBLISH_PREVIEWS === '1') {
    await board.screenshot({path: `public/generated-flyers/brunch-sundays-${publishFormat}-preview.png`, animations: 'disabled'});
  }
...
try {
  ...
  for (const format of formats) {
    ...
    assert.doesNotMatch(font, /PNG/, 'Brunch uses editable live type with texture');
  ...
  if (runControls) {
    ...
    for (const format of formats) {
      await ready(format, initialMaps[format]);
      const checkObjects=process.env.NF_BRUNCH_SUNDAYS_CHECK_OBJECTS?.split(',');
