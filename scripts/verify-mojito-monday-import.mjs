...
try{
 ...
 for(const [id,file] of [['mojito-monday','mojito-monday.nflyer']]){
   ...
   if(process.env.NF_CASE_ONLY){
     ...
     const original=await input.inputValue();assert.equal(original,'MOJITo');
     await input.fill('o');await input.press('ArrowLeft');await input.press('Shift+O');
     assert.equal(await input.inputValue(),'Oo');
     await page.waitForTimeout(600);
     assert.match((await page.locator('[data-coco-compiled-object="headline"]').textContent()).replace(/\s/g,''),/^(Oo)+$/);
     await page.screenshot({clip:await page.locator('#artboard').boundingBox(),path:out+'/case-sensitive-O.png'});
     await input.fill(original);
     console.log('SHIFT O AND LOWERCASE o PASSED');
   ...
   if(!process.env.NF_ROUNDTRIP_ONLY) for(const format of ['square','story']){
     ...
     assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).replace(/\s/g,'').includes('MOJITo'));
     ...
     await page.waitForFunction((format)=>{
       ...
       return board?.textContent.replace(/\s/g,'').includes('MOJITo') && !/Preparing (?:story|square) canvas/.test(board.textContent) && rect && (format==='story'?rect.height/rect.width>1.7:Math.abs(rect.height/rect.width-1)<0.05);
     ...
     if(!process.env.NF_PREVIEW_ONLY)for(const objectId of ["presenter","headline","subtitle","day","date","year","genres","mood","description","service1","service2","service3","venue","address","qrCaption","motto"]){
       ...
       if(objectId==='headline'){
         const controls=page.locator('#headline-panel');
         ...
         assert.equal(await menu.getByRole('button',{name:/Mojito Serif/,includeHidden:true}).count(),1,'Mojito family available in shared options');
         assert.equal(await controls.locator('[data-png-glyph-picker]').getByRole('button',{name:/Mojito Serif/,includeHidden:true}).count(),1,'Mojito family available in PNG gallery');
         await picker.locator('button').first().evaluate(el=>el.click());
       }
 ...
 await page.waitForTimeout(2000);
 await page.locator('[aria-label="Preparing flyer canvas"]').waitFor({state:'hidden',timeout:90000});
 assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).includes('MOJITo'));
 ...
 for(const format of ['Square','Story']){
   ...
   await page.screenshot({clip:await page.locator('#artboard').boundingBox(),path:`${out}/mojito-monday-${format.toLowerCase()}-roundtrip.png`});
 }
 if(process.env.NF_EXPORT_CHECKS){
   ...
   for(const format of ['Square','Story']){
     ...
     await writeFile(`${out}/mojito-monday-${format.toLowerCase()}-export.png`,Buffer.from(data));
