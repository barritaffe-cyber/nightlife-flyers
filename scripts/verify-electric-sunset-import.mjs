...
try{
 ...
 for(const [id,file] of [['electric-sunset','electric-sunset.nflyer']]){
   ...
   if(!process.env.NF_ROUNDTRIP_ONLY) for(const format of ['square','story']){
     ...
     assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).replace(/\s/g,'').includes('SUN'));
     ...
     await page.waitForFunction((format)=>{
       ...
       return board?.textContent.replace(/\s/g,'').includes('SUN') && !/Preparing (?:story|square) canvas/.test(board.textContent) && rect && (format==='story'?rect.height/rect.width>1.7:Math.abs(rect.height/rect.width-1)<0.05);
     ...
     if(!process.env.NF_PREVIEW_ONLY)for(const objectId of ["presenter", "headline", "subtitle", "day", "date", "year", "tagline", "mood", "genres", "venue", "address", "perks", "dress", "qrCaption"]){
 ...
 await page.waitForTimeout(2000);
 await page.locator('[aria-label="Preparing flyer canvas"]').waitFor({state:'hidden',timeout:90000});
 assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).includes('SUN'));
 ...
 for(const format of ['Square','Story']){
   ...
   await page.screenshot({clip:await page.locator('#artboard').boundingBox(),path:`${out}/electric-sunset-${format.toLowerCase()}-roundtrip.png`});
 }
 if(process.env.NF_EXPORT_CHECKS){
   ...
   for(const format of ['Square','Story']){
     ...
     await writeFile(`${out}/electric-sunset-${format.toLowerCase()}-export.png`,Buffer.from(data));
