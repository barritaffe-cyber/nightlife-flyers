...
const out = process.env.NF_OUTPUT_DIR || '/tmp/ycee-live-discovery';
const eventName = process.env.NF_EVENT_NAME || 'YCEE Live';
const theme = process.env.NF_THEME || 'Elegant';
const recipeId = process.env.NF_RECIPE_ID || 'ycee-live';
...
try {
  ...
  assert.equal(await card.locator('[data-portrait-edge-fade="true"]').count(), 4, 'both portraits fade in both personalized previews');
  ...
  await card.getByRole('button', { name: 'Choose this design', exact: true }).click();
  await page.getByTestId('coco-business-setup').waitFor();
  ...
  console.log(`PASS ${eventName} / ${theme} through both personalized previews to the details form.`);
} catch (error) {
