...
try{
 ...
 for(const [id,file] of [['branch-happy-hour','branch-happy-hour.nflyer']]){
   ...
   if(!process.env.NF_ROUNDTRIP_ONLY) for(const format of ['square','story']){
     ...
     if(!process.env.NF_PREVIEW_ONLY)for(const objectId of ["presenter", "brand", "brandSub", "headline", "subtitle", "date", "hours", "discount", "genres", "mood", "venue", "address", "reservation", "motto"]){
 ...
 await page.waitForTimeout(2000);
 await page.locator('[aria-label="Preparing flyer canvas"]').waitFor({state:'hidden',timeout:90000});
 assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).includes('HAPPY'));
 ...
 for(const format of ['Square','Story']){
   ...
   await page.screenshot({clip:await page.locator('#artboard').boundingBox(),path:`${out}/branch-happy-hour-${format.toLowerCase()}-roundtrip.png`});
 }
 if(process.env.NF_EXPORT_CHECKS){
   ...
   for(const format of ['Square','Story']){
     ...
     await writeFile(`${out}/branch-happy-hour-${format.toLowerCase()}-export.png`,Buffer.from(data));
