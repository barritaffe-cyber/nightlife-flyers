...
try{
 ...
 for(const [id,file] of [['sunset-yacht','sunset-yacht.nflyer']]){
   ...
   if(!process.env.NF_ROUNDTRIP_ONLY) for(const format of ['square','story']){
     ...
     assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).replace(/\s/g,'').includes('SUNSET'));
     assert.equal(await page.locator('[data-coco-compiled-object="subtitle"]').count(),1);
     assert.doesNotMatch(await page.locator('[data-coco-compiled-document="true"]').innerText(), /SUB HEADLINE|MIDNIGHT MUSE|NIGHTLIFE FLYERS|FREE PARKING/);
     ...
     await page.waitForFunction((format)=>{
       ...
       return board?.textContent.replace(/\s/g,'').includes('SUNSET') && !/Preparing (?:story|square) canvas/.test(board.textContent) && rect && (format==='story'?rect.height/rect.width>1.7:Math.abs(rect.height/rect.width-1)<0.05);
     ...
     if(!process.env.NF_PREVIEW_ONLY)for(const objectId of ["presenter","presents","headline","subtitle","tagline","day","month","date","time","venue","club","address","mood","views","invitation"]){
 ...
 if(['mood','address'].includes(objectId)){
         const panel=objectId==='mood'?'details':'venue';
       ...
       if(objectId==='mood'){
         ...
         assert.ok(Math.abs(tracking-.28)<.001,format+' preserves authored details tracking: '+tracking);
 ...
 await page.waitForTimeout(2000);
 await page.locator('[aria-label="Preparing flyer canvas"]').waitFor({state:'hidden',timeout:90000});
 assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).includes('SUNSET'));
 assert.equal(await page.locator('[data-coco-compiled-object="address"]').getAttribute('data-coco-compiled-align'),'left');
 console.log('PROJECT ROUNDTRIP PASSED');
 for(const format of ['Square','Story']){
   await page.getByRole('button',{name:format,exact:true}).click();
   await page.waitForTimeout(1200);
   await page.locator('[aria-label="Preparing flyer canvas"]').waitFor({state:'hidden',timeout:90000});
   const tracking=await page.locator('[data-coco-compiled-object="mood"]').evaluate(el=>{const style=getComputedStyle(el);return parseFloat(style.letterSpacing)/parseFloat(style.fontSize);});
   assert.ok(Math.abs(tracking-.28)<.001,format+' retains details tracking after reopening: '+tracking);
   await page.screenshot({clip:await page.locator('#artboard').boundingBox(),path:`${out}/sunset-yacht-${format.toLowerCase()}-roundtrip.png`});
 }
 if(process.env.NF_EXPORT_CHECKS){
   ...
   for(const format of ['Square','Story']){
     ...
     await writeFile(`${out}/sunset-yacht-${format.toLowerCase()}-export.png`,Buffer.from(data));
