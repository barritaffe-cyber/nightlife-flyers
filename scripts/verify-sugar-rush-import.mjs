...
try{
 ...
 for(const [id,file] of [['sugar-rush','sugar-rush.nflyer']]){
   ...
   if(!process.env.NF_ROUNDTRIP_ONLY) for(const format of ['square','story']){
     ...
     assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).replace(/\s/g,'').includes('SUGAR'));
     ...
     await page.waitForFunction((format)=>{
       ...
       return board?.textContent.replace(/\s/g,'').includes('SUGAR') && !/Preparing (?:story|square) canvas/.test(board.textContent) && rect && (format==='story'?rect.height/rect.width>1.7:Math.abs(rect.height/rect.width-1)<0.05);
     ...
     if(!process.env.NF_PREVIEW_ONLY)for(const objectId of ["presenter", "headline", "subtitle", "day", "date", "year", "genres", "mood", "offer", "offerTime", "description", "dj1", "dj2", "dj3", "venue", "address", "reservation", "vibes", "qrCaption"]){
       ...
       if(objectId==='headline'){
         ...
         assert.equal(await menu.getByRole('button',{name:/Ladies Neon Chrome/,includeHidden:true}).count(),0,'Template-only font absent from shared font options');
         assert.equal(await controls.locator('[data-png-glyph-picker]').getByRole('button',{name:/Ladies Neon Chrome/,includeHidden:true}).count(),0,'Template-only font absent from PNG gallery');
         await picker.locator('button').first().evaluate(el=>el.click());
       }
 ...
 await page.waitForTimeout(2000);
 await page.locator('[aria-label="Preparing flyer canvas"]').waitFor({state:'hidden',timeout:90000});
 assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).includes('SUGAR'));
 ...
 for(const format of ['Square','Story']){
   ...
   await page.screenshot({clip:await page.locator('#artboard').boundingBox(),path:`${out}/sugar-rush-${format.toLowerCase()}-roundtrip.png`});
 }
 if(process.env.NF_EXPORT_CHECKS){
   ...
   for(const format of ['Square','Story']){
     ...
     await writeFile(`${out}/sugar-rush-${format.toLowerCase()}-export.png`,Buffer.from(data));
