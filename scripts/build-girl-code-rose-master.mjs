...
const semanticRoles = {...Object.fromEntries(Object.entries(native).map(([id,semanticRole])=>[id,{semanticRole}])),...Object.fromEntries(Object.entries(companions).map(([id,[semanticRole,panel]])=>[id,{semanticRole,text:id==='djLabel'?'djLineupLabel':`girlCodeRose_${id}`,panel,uiField:id==='djLabel'?'djLineupLabel':panel,moveTarget:panel,mappedControls:true,editable:true}])),background:{semanticRole:'background',editable:false},...Object.fromEntries(['contactPlate','legibility','rMood','rMotto','rDate','rDjs','rVenue','crown','heart'].map(id=>[id,{editable:false}]))};
const fontMap = {Rose:'Rose Chrome Serif PNG',Script:'Dear Script (Demo_Font)',RoseSerif:'Avigea',Hand:'Northwell',Sans:'LEMONMILK-Light',Medium:'LEMONMILK-Regular',...Object.fromEntries(Object.keys(semanticRoles).map(id=>[id,id==='headline'?'Rose Chrome Serif PNG':id==='subtitle'?'Dear Script (Demo_Font)':['date','venue'].includes(id)?'Avigea':id==='tagline'?'Northwell':['day','month','time','dj1','dj2','dj3','address','contact'].includes(id)?'LEMONMILK-Regular':'LEMONMILK-Light']))};
export const cocoCssMasterAdapter = {
  id:'girl-code-rose', masterPath:new URL('../public/generated-flyers/girl-code-rose-master.html',import.meta.url),
  outputPath:new URL('../public/generated-flyers/girl-code-rose.nflyer',import.meta.url), publicRoot:new URL('../public/',import.meta.url),
  editorTextScale:.5, requiredFonts:['Rose','Script','RoseSerif','Hand','Sans','Medium'], requiredRoles:Object.values(native), semanticRoles, fontMap,
  recipe:{id:'girl-code-rose',name:'Girl Code — Rose',version:1,summary:'Rose chrome lettering above a sunset rooftop portrait, with independent event information.',runtime:{
    compositionPattern:'center-poster-stack',styleId:'black-electric',
    palette:{bgFrom:'#49242a',bgTo:'#99554f',primary:'#f5b0be',secondary:'#ffe4dc',accent:'#d98496',neutral:'#ffe4dc'},
...
export async function buildGirlCodeRoseMaster() {
  if (await access(new URL('../lib/template-data/girl-code-rose-saved-source.json',import.meta.url)).then(()=>true,()=>false)) {
    throw new Error('Girl Code — Rose has an accepted user save; preserve it instead of rebuilding.');
  }
  ...
  for (const v of [result.square,result.story]) {
    ...
    const bounds = {x:62/10.8,y:v===result.story?1118/19.2:605/10.8,width:26,height:1};
    doc.objects.push({...structuredClone(body),id:'detailsLabel',sourceObjectId:'detailsLabel',semanticRole:'detailsLabel',text:'',textRuns:[],bounds,paintBounds:bounds,
      ...
      binding:{text:'detailsLabel',size:'detailsLabelSize',color:'detailsLabelColor',family:'detailsFamily',panel:'details',moveTarget:'details',uiField:'detailsLabel',mappedControls:true,initial:{text:'',size:6,color:'#ffe4dc'}},
    });
    v.detailsLabel='';v.detailsLabelSize=6;v.detailsLabelColor='#ffe4dc';v.detailsTracking=v.bodyTracking;
  ...
  await writeFile(new URL('../lib/template-data/girl-code-rose-v1.json',import.meta.url),JSON.stringify({square:result.square,story:result.story},null,2)+'\n');
  console.log(JSON.stringify({square:result.square.cocoCssCompiler.report,story:result.story.cocoCssCompiler.report}));
  return result;
}
if(process.argv[1] && import.meta.url===pathToFileURL(process.argv[1]).href)await buildGirlCodeRoseMaster();
