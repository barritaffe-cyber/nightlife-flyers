...
configs={'euphoria-chrome':('euphoria','Euphoria Chrome','EuphoriaChromePNG',[])}
...
if key=='euphoria-chrome':
 # B and D have disconnected left strokes; group by explicit source cells.
 cells=[('ABCDEFG',[40,278,470,676,899,1072,1244,1475],0,216,201,179),
 ('HIJKLMN',[70,306,402,591,807,992,1238,1475],216,418,405,180),
 ('OPQRSTU',[35,294,480,686,893,1072,1260,1490],418,629,614,183),
 ('VWXYZ',[165,391,662,901,1123,1360],629,822,811,172),
 ('0123456789',[15,187,274,431,573,720,889,1040,1184,1338,1520],822,1024,990,153)]
...
for ch,points in assigned.items():
 ...
 for y in range(t,b):
  for x in range(l,r):
   ...
   if alpha:px[x-l,y-t]=value[:3]+(max(0,round((value[3]-65)*255/190)),)
   else:
