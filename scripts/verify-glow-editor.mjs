...
const out = process.env.NF_OUTPUT_DIR || (process.env.NF_ACCEPTED_PREVIEW ? 'public/generated-flyers' : '/tmp/glow-editor');
...
try{
 ...
 for(const file of ['glow.nflyer']){
   ...
   console.log('READY');
   const lateChoice=page.getByRole('button',{name:'Choose Square format',exact:true});if(await lateChoice.isVisible()){await lateChoice.click();const keep=page.getByRole('button',{name:'Keep this layout',exact:true});if(await keep.isVisible())await keep.click();await page.locator('input[type=file][accept*=json]').setInputFiles('public/generated-flyers/glow.nflyer');await page.waitForTimeout(2000);}
   ...
   for(const format of ['Square','Story']){
     ...
     await page.waitForFunction(format=>{
       ...
       const owners=Array.from(document.querySelectorAll('[data-coco-compiled-object]')).filter(el=>el.querySelector('[data-text-hit-surface="true"]') && el.textContent.trim());
       return owners.length===16 && box && (format==='Story'?box.height/box.width>1.7:Math.abs(box.height/box.width-1)<.05);
     },format,{timeout:60000});
     await page.evaluate(()=>document.fonts.ready);
     await page.waitForTimeout(1000);
     const artboardBox=await page.locator('#artboard').boundingBox();
     await page.mouse.click(artboardBox.x+3,artboardBox.y+3);await page.waitForTimeout(300);
     await page.locator('#artboard').screenshot({path:out+'/glow-'+format.toLowerCase()+'-preview.png'});
     if(process.env.NF_PREVIEW_ONLY)continue;
     // Each visible compiled text owner must be selectable independently.
     const visibleIds=await page.locator('[data-coco-compiled-object]').evaluateAll(nodes=>nodes.filter(el=>el.querySelector('[data-text-hit-surface="true"]') && el.textContent.trim()).map(el=>el.getAttribute('data-coco-compiled-object')));
     assert.equal(visibleIds.length,16,'All visible text owners loaded');
     for(const id of visibleIds){
       const owner=page.locator(`[data-coco-compiled-object="${id}"]`);
       await owner.evaluate(el=>el.click());await page.waitForTimeout(80);
       assert.equal(await owner.getAttribute('data-active'),'true','Selectable text owner '+id);
     }
     console.log('TEXT OWNERS SELECTED',format,visibleIds.length);
...
assert.equal(await owner.getAttribute('data-active'),'true');
       ...
       const edit=page.getByPlaceholder(objectId==='headline'?'ENTER HEADLINE...':'Optional sub-headline',{exact:true});
       if(objectId==='headline') {
         const glyphs=owner.locator('[data-png-glyph-layout="glow-offset-v1"]');
         assert.equal(await glyphs.count(),4);
         const transforms=await glyphs.evaluateAll(nodes=>nodes.map(el=>el.style.transform));
         for(const [i,scale] of [1,.92,.96,1.04].entries())assert.ok(transforms[i].includes(`scale(${scale})`));
         for(const [i,angle] of [0,-1,1,2].entries())assert.ok(transforms[i].includes(`rotate(${angle}deg)`));
       }
       const originalText=await edit.inputValue();
       await edit.fill('GLOW TEST');await page.waitForTimeout(400);
       assert.ok((await owner.textContent()).includes('GLOW TEST'));
       await edit.fill(originalText);await page.waitForTimeout(400);
       ...
       const paint=()=>owner.evaluate(el=>Array.from(el.querySelectorAll('span, svg, g, text, path, [data-headline-shadow-glyph]')).map(n=>{const s=getComputedStyle(n);return s.filter+'|'+s.textShadow}).join(';'));
       ...
       if(objectId==='headline') {
         const glyph=owner.locator('[data-png-glyph-run] [data-headline-shadow-glyph]').first();
         const filter=await glyph.evaluate(el=>getComputedStyle(el).filter);
         assert.match(filter,/url\(/,'PNG shadow uses a core-alpha SVG filter');
         const run=owner.locator('[data-png-glyph-run]').first();
         assert.equal(await run.evaluate(el=>getComputedStyle(el).filter),'none','No full-word shadow');
         const letterFilters=await run.locator('[data-headline-shadow-glyph]').evaluateAll(nodes=>nodes.filter(el=>el.textContent.trim()).map(el=>getComputedStyle(el).filter));
         assert.ok(letterFilters.length>1 && letterFilters.every(value=>value.includes('url(')),'Every letter filters its own alpha');
         const shadowFilter=owner.locator('[data-png-shadow-filter]').first();
         assert.equal(await shadowFilter.locator('feFuncA').getAttribute('type'),'gamma','Retain smooth glyph alpha');
         const dx=Number(await shadowFilter.locator('feOffset').getAttribute('dx'));
         const dy=Number(await shadowFilter.locator('feOffset').getAttribute('dy'));
         const blur=Number(await shadowFilter.locator('feGaussianBlur').getAttribute('stdDeviation'));
         assert.ok(blur>0 && dy>blur*2 && dy>dx,'Soft shadow remains directional');
         assert.equal(await glyph.evaluate(el=>getComputedStyle(el).textShadow),'none','No extra live text-shadow halo');
         await owner.screenshot({path:out+'/'+format.toLowerCase()+'-png-shadow.png'});
       }
 ...
 if(process.env.NF_PREVIEW_ONLY){await browser.close();process.exit(0);}
 ...
 for(const format of ['Square','Story','Square']){
   ...
   for(const [id,panel] of [['headline','headline'],['subtitle','head2']]){
     ...
     if(id==='headline')assert.equal(await page.locator('[data-png-glyph-layout="glow-offset-v1"]').count(),4,'Letter layout persists after save/reopen');
