...
configs={'ladies-chrome':('LN-FAM','Ladies Neon Chrome','LadiesNeonChromePNG',[])}
...
if key=='ladies-chrome':
 # B and D have disconnected left strokes; group by explicit source cells.
 cells=[('ABCDEFG',[0,257,465,670,879,1063,1254,1536],0,225,216,199),
 ('HIJKLMN',[0,266,364,558,776,1009,1269,1536],225,440,430,197),
 ('OPQRSTU',[0,254,438,670,899,1078,1287,1536],440,657,644,197),
 ('VWXYZ',[140,376,655,896,1178,1375],657,841,834,171),
 ('0123456789',[10,184,270,434,582,729,882,1035,1187,1352,1527],841,1024,1007,158)]
...
# These three numeral cells contain tiny disconnected tips from adjacent glyphs.
# Keep their single complete numeral component, without neighboring fragments.
for ch in '123':
 todo=set(assigned[ch]);parts=[]
 while todo:
  p=todo.pop();queue=[p];part={p}
  while queue:
   x,y=queue.pop()
   for n in ((x-1,y),(x+1,y),(x,y-1),(x,y+1)):
    if n in todo:todo.remove(n);queue.append(n);part.add(n)
  parts.append(part)
...
for ch,points in assigned.items():
 ...
 horizontal_scale=.78 if ch=='L' else 1
 if horizontal_scale!=1:im=im.resize((round(im.width*horizontal_scale),im.height),Image.Resampling.LANCZOS)
 advance=round((core_bounds[ch][2]-core_bounds[ch][0])*scale*5*horizontal_scale)+45
 # Tighten only the advance after L; preserve all other letter spacing.
 if ch=='L':advance-=55
 ...
 profile={}
 for x,y in points:
  row=round((baseline-y)*scale/2);left=(x-l-pad)*scale*5*horizontal_scale;right=(core_bounds[ch][2]-1-x)*scale*5*horizontal_scale
  if row in profile:profile[row]=(min(profile[row][0],left),min(profile[row][1],right))
  else:profile[row]=(left,right)
 profiles[name]=profile;report[ch]=dict(advance=advance,sourceBounds=[l,t,r,b],sourceScale=scale,horizontalScale=horizontal_scale,sourceBaseline=baseline,bitmapOffsetX=ox,bitmapOffsetY=oy,bitmapSize=im.size)
