"""Extract supplied Honey Gold Serif sheets and package editable UI bitmap lettering.

Run with python3 scripts/build-honey-gold-serif-font.py.
...
OUT = ROOT / 'public/generated-flyers/assets/png-glyphs/honey-gold-serif'
...
for sheet_index, chars in enumerate(SHEETS, 1):
    path = SOURCE_DIR / f'honey{sheet_index:02d}.png'
    ...
    for start, opacity in enumerate(raw):
        ...
        while stack:
            ...
            for nxt in adjacent:
                if not seen[nxt] and raw[nxt] >= 240:
                    seen[nxt] = 1
                    stack.append(nxt)
        if len(points) < 12:
            continue
    ...
    for index, char in enumerate(chars):
        ...
        descender = 200 if char in 'gjpqy' else {'G':60,'J':130,'Y':180,'f':70}.get(char,0)
...
FONT.setupNameTable({'familyName':'Honey Gold Serif PNG','styleName':'Regular','uniqueFontIdentifier':'HoneyGoldSerifPNG-1','fullName':'Honey Gold Serif PNG','psName':'HoneyGoldSerifPNG','version':'Version 1.000'})
...
FONT.font.flavor='woff2';FONT.save(ROOT/'public/fonts/HoneyGoldSerifPNG.woff2')
(OUT/'metrics.json').write_text(json.dumps({'family':'Honey Gold Serif PNG','version':1,'bitmapPpem':PPEM,'sources':SOURCES,'glyphs':REPORT,'kerningPairs':len(pairs)},indent=2)+'\n')
