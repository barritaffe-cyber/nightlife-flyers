...
try{
 ...
 for(const [id,file] of [['neon-night','neon-night.nflyer']]){
   ...
   if(!process.env.NF_ROUNDTRIP_ONLY) for(const format of ['square','story']){
     ...
     if(!process.env.NF_PREVIEW_ONLY)for(const objectId of ["headline", "subtitle", "presenter", "presents", "day", "month", "date", "doors", "hours", "genres", "offers", "lineupLabel", "dj1", "dj2", "dj3", "address", "dress", "age", "idLabel"]){
 ...
 await page.waitForTimeout(2000);
 await page.locator('[aria-label="Preparing flyer canvas"]').waitFor({state:'hidden',timeout:90000});
 assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).includes('NIGHT'));
 ...
 for(const format of ['Square','Story']){
   ...
   await page.screenshot({clip:await page.locator('#artboard').boundingBox(),path:`${out}/neon-night-${format.toLowerCase()}-roundtrip.png`});
 }
 if(process.env.NF_EXPORT_CHECKS){
   ...
   for(const format of ['Square','Story']){
     ...
     await writeFile(`${out}/neon-night-${format.toLowerCase()}-export.png`,Buffer.from(data));
