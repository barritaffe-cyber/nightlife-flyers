...
const companions={"day": ["day", "date", "date"], "year": ["year", "date", "date"], "mood": ["mood", "details", "details"], "service1": ["service1", "details", "details"], "service2": ["service2", "details", "details"], "service3": ["service3", "details", "details"], "qrCaption": ["qrCaption", "details", "details"], "motto": ["motto", "details", "details"]};
const semanticRoles={...Object.fromEntries(Object.entries(native).map(([id,semanticRole])=>[id,{semanticRole}])),...Object.fromEntries(Object.entries(companions).map(([id,[semanticRole,panel,uiField]])=>[id,{semanticRole,text:id==='detailsLabel'?'detailsLabel':`mojitoMonday_${id}`,panel,uiField,moveTarget:panel,mappedControls:true,editable:true}]))};
const fontMap={Mint:'Mojito Serif PNG',TieSerif:'Didot',Condensed:'Bebas Neue',Sans:'LEMONMILK-Light',...Object.fromEntries(Object.keys(semanticRoles).map(id=>[id,id==='headline'?'Mojito Serif PNG':id==='venue'?'Didot':['day','date'].includes(id)?'Bebas Neue':'LEMONMILK-Light']))};
export const cocoCssMasterAdapter={
 id:'mojito-monday',masterPath:new URL('../public/generated-flyers/mojito-monday-master.html',import.meta.url),outputPath:new URL('../public/generated-flyers/mojito-monday.nflyer',import.meta.url),publicRoot:new URL('../public/',import.meta.url),editorTextScale:.5,requiredFonts:['Mint','TieSerif','Condensed','Sans'],requiredRoles:Object.values(native),semanticRoles,fontMap,
 recipe:{id:'mojito-monday',name:'Mojito Monday — Mint Lounge',version:2,runtime:{compositionPattern:'center-poster-stack',styleId:'black-electric',palette:{bgFrom:'#070707',bgTo:'#171714',primary:'#f9f6eb',secondary:'#aaa69b',accent:'#dcc38e',neutral:'#edece5'},authority:{layout:{owner:'recipe-elements'},palette:{owner:'recipe-element-paint'},assets:{owner:'recipe-elements'},crop:{owner:'recipe-background-image'}},formats:{square:{canvas:{width:1080,height:1080}},story:{canvas:{width:1080,height:1920}}}}},eventBrief:{eventName:'MOJITo',subtitle:'MONDAY',date:'DEC 1'}
};
if(await access(new URL('../lib/template-data/mojito-monday-saved-source.json',import.meta.url)).then(()=>true,()=>false))throw Error('Mojito Monday has an accepted user save; preserve it instead of rebuilding.');
...
for(const v of [result.square,result.story]){
 Object.assign(v,{backgroundUrl:blank,bgUrl:blank,subtagEnabled:false,rightRailEnabled:true,rightRailLabel:'',presenterEnabled:true,priceEnabled:false,priceLabel:'',qrEnabled:true,qrImageUrl:null,qrX:v===result.story?83:80.5,qrY:v===result.story?88:80.5,qrScale:7.5/11,leftRailEnabled:false,leftRail:'',leftRailLabel:'',headShadow:false,detailsShadow:false,venueShadow:false,socialHandleEnabled:false,cocoSocialHandleEnabled:false,details2Enabled:false,details2:'',djLineupLabel:'',exp:1,contrast:1/.9,saturation:1,warmth:0,tint:0,gamma:1,grain:0,vibrance:0,filmGrade:0});
 ...
 const bounds={x:7.5,y:v===result.story?77:46,width:45,height:2};
...
await writeFile(new URL('../lib/template-data/mojito-monday-v2.json',import.meta.url),JSON.stringify({square:result.square,story:result.story},null,2)+'\n');
