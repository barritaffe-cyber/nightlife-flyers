...
const native={"headline":"headline","subtitle":"headline2","tagline":"subtag","date":"date","time":"time","venue":"venue","address":"address","genres":"details","invitation":"footerDetails"};
const companions={"venueLabel":["venueLabel","venue","venueAddress"],"day":["weekday","date","date"],"month":["month","date","date"],"club":["venueDescriptor","venue","venueAddress"],"mood":["mood","details","details"],"views":["views","details","details"],"detailsLabel":["detailsLabel","details","detailsLabel"]};
const semanticRoles={...Object.fromEntries(Object.entries(native).map(([id,semanticRole])=>[id,{semanticRole}])),...Object.fromEntries(Object.entries(companions).map(([id,[semanticRole,panel,uiField]])=>[id,{semanticRole,text:id==='detailsLabel'?'detailsLabel':`afrobeat_${id}`,panel,uiField,moveTarget:panel,mappedControls:true,editable:true}]))};
const serif=new Set(['headline','venue']);
...
export const cocoCssMasterAdapter={
 id:'afrobeat-rooftop',masterPath:new URL('../public/generated-flyers/afrobeat-rooftop-master.html',import.meta.url),outputPath:new URL('../public/generated-flyers/afrobeat-rooftop.nflyer',import.meta.url),publicRoot:new URL('../public/',import.meta.url),editorTextScale:.5,requiredFonts:['TieSerif','TieScript','TieSans','TieBold'],requiredRoles:Object.values(native),semanticRoles,fontMap,
 recipe:{id:'afrobeat-rooftop',name:'Afrobeat Rooftop Sessions',version:2,runtime:{compositionPattern:'center-poster-stack',styleId:'black-electric',palette:{bgFrom:'#070707',bgTo:'#171714',primary:'#f9f6eb',secondary:'#aaa69b',accent:'#dcc38e',neutral:'#edece5'},authority:{layout:{owner:'recipe-elements'},palette:{owner:'recipe-element-paint'},assets:{owner:'recipe-elements'},crop:{owner:'recipe-background-image'}},formats:{square:{canvas:{width:1080,height:1080}},story:{canvas:{width:1080,height:1920}}}}},eventBrief:{eventName:'AFROBEAT',subtitle:'Sessions',date:'OCT 12'}
};
if(await access(new URL('../lib/template-data/afrobeat-rooftop-saved-source.json',import.meta.url)).then(()=>true,()=>false))throw Error('Afrobeat Rooftop has an accepted user save; preserve it instead of rebuilding.');
...
for(const v of [result.square,result.story]){
 Object.assign(v,{backgroundUrl:blank,bgUrl:blank,presenterEnabled:false,presenter:'',priceEnabled:false,priceLabel:'',qrEnabled:false,leftRailEnabled:false,leftRail:'',leftRailLabel:'',headShadow:false,socialHandleEnabled:false,cocoSocialHandleEnabled:false,details2Enabled:false,details2:'',djLineupLabel:'',exp:1,contrast:1/.9,saturation:1,warmth:0,tint:0,gamma:1,grain:0,vibrance:0,filmGrade:0});
 ...
 const bounds={x:20,y:v===result.story?71:79,width:60,height:2};
 ...
 doc.objects=doc.objects.map(withCompiledTextSelection).map(object=>{
   ...
   if(object.id==='headline') {object.paint.backgroundImage='url("/generated-flyers/assets/afrobeat-rooftop-gold.svg")';object.paint.backgroundSize=v===result.story?'1037px 191px':'1037px 183px';}
...
await writeFile(new URL('../lib/template-data/afrobeat-rooftop-v2.json',import.meta.url),JSON.stringify({square:result.square,story:result.story},null,2)+'\n');
