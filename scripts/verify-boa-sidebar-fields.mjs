...
try{
 ...
 for(const [id,file] of [['como-una-boa','como-una-boa.nflyer']]){
   ...
   for(const format of ['square','story']){
     ...
     for (const [panel,placeholder,value,objectId] of [
       ['venue',null,'CLUB BOA','addressLabel'],
       ['details2','MUSIC BY','MUSIC BY DJ TEST','lineupLabel'],
       ['details','EVENT DETAILS','HOSTED BY TEST','hostLabel'],
     ]) {
       const region=page.locator('#'+panel+'-panel');
       const title={venue:'Venue',details2:'DJ Lineup',details:'Event Details'}[panel];
       const input=placeholder ? region.getByPlaceholder(placeholder,{exact:true}) : region.locator('input').filter({visible:true}).first();
       await page.getByRole('button',{name:'▸ '+title,exact:true}).click();
       const target=placeholder ? input : region.locator('label').filter({hasText:'Venue Name'}).first().locator('..').locator('input').first();
       await target.fill(value+' '+format);
       await page.waitForFunction(({id,value})=>document.querySelector('[data-coco-compiled-object="'+id+'"]')?.textContent?.includes(value),{id:objectId,value:value+' '+format},{timeout:10000});
       assert.equal(await target.inputValue(),value+' '+format);
       console.log('EDITED',format,panel);
     }
