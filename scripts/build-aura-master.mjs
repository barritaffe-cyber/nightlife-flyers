...
const companions={day:['weekday','date','date'],month:['month','date','date'],year:['year','date','date'],handle:['handle','details','details'],entry:['entry','details','details'],entryLabel:['entryLabel','details','details'],parking:['parking','details','details'],rsvp:['reservationInfo','details','details'],qrCaption:['qrCaption','details','details'],detailsLabel:['detailsLabel','details','detailsLabel']};
const semanticRoles={...Object.fromEntries(Object.entries(native).map(([id,semanticRole])=>[id,{semanticRole}])),...Object.fromEntries(Object.entries(companions).map(([id,[semanticRole,panel,uiField]])=>[id,{semanticRole,text:id==='detailsLabel'?'detailsLabel':`aura_${id}`,panel,uiField,moveTarget:panel,mappedControls:true,editable:true}]))};
const fontMap={Gold:'Gold Flourish PNG',TieSerif:'Didot',Sans:'LEMONMILK-Light',...Object.fromEntries(Object.keys(semanticRoles).map(id=>[id,id==='headline'?'Gold Flourish PNG':['month','date','venue','entry'].includes(id)?'Didot':'LEMONMILK-Light']))};
export const cocoCssMasterAdapter={
 id:'aura',masterPath:new URL('../public/generated-flyers/aura-master.html',import.meta.url),outputPath:new URL('../public/generated-flyers/aura.nflyer',import.meta.url),publicRoot:new URL('../public/',import.meta.url),editorTextScale:.5,requiredFonts:['Gold','TieSerif','Sans'],requiredRoles:Object.values(native),semanticRoles,fontMap,
 recipe:{id:'aura',name:'Aura — Elevated Nightlife',version:2,runtime:{compositionPattern:'center-poster-stack',styleId:'black-electric',palette:{bgFrom:'#070707',bgTo:'#171714',primary:'#f9f6eb',secondary:'#aaa69b',accent:'#dcc38e',neutral:'#edece5'},authority:{layout:{owner:'recipe-elements'},palette:{owner:'recipe-element-paint'},assets:{owner:'recipe-elements'},crop:{owner:'recipe-background-image'}},formats:{square:{canvas:{width:1080,height:1080}},story:{canvas:{width:1080,height:1920}}}}},eventBrief:{eventName:'AURA',subtitle:'ELEVATED NIGHTLIFE',date:'MAY 26TH 2026'}
};
if(await access(new URL('../lib/template-data/aura-saved-source.json',import.meta.url)).then(()=>true,()=>false))throw Error('Aura has an accepted user save; preserve it instead of rebuilding.');
...
for(const v of [result.square,result.story]){
 Object.assign(v,{backgroundUrl:blank,bgUrl:blank,subtagEnabled:false,rightRailEnabled:true,rightRailLabel:'',presenterEnabled:true,priceEnabled:false,priceLabel:'',qrEnabled:true,qrImageUrl:null,qrX:v===result.story?87:88.3,qrY:v===result.story?17:22.5,qrScale:7/11,leftRailEnabled:false,leftRail:'',leftRailLabel:'',headShadow:false,socialHandleEnabled:false,cocoSocialHandleEnabled:false,details2Enabled:false,details2:'',djLineupLabel:'',exp:1,contrast:1/.9,saturation:1,warmth:0,tint:0,gamma:1,grain:0,vibrance:0,filmGrade:0});
 ...
 const bounds={x:4,y:v===result.story?17.5:21.8,width:30,height:2};
...
await writeFile(new URL('../lib/template-data/aura-v2.json',import.meta.url),JSON.stringify({square:result.square,story:result.story},null,2)+'\n');
