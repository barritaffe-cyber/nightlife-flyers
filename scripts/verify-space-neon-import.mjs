...
const out = '/private/tmp/space-neon-verification';
...
try{
 for(const format of ['square','story']){
  await page.goto(`http://localhost:3000/generated-flyers/space-neon-master.html?format=${format}`,{waitUntil:'networkidle'});
 ...
 for(const [id,file] of [['space-neon','space-neon.nflyer']]){
   ...
   for(const format of ['square','story']){
     ...
     for(const objectId of ['presenter','day','month','guestLabel','guest','offer','lineup','headline','space','subtag','address','contact']){
       const textNode=page.locator(`[data-coco-compiled-object="${objectId}"]`);
       await textNode.locator('[data-text-hit-surface="true"]').first().click({noWaitAfter:true,timeout:15000});
