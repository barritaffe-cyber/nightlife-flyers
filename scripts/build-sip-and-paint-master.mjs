...
const companions={"and": ["and", "details", "details"], "day": ["day", "date", "date"], "hours": ["hours", "date", "date"], "mood": ["mood", "details", "details"], "tagline": ["tagline", "details", "details"], "cost": ["cost", "details", "details"], "perPerson": ["perPerson", "details", "details"], "included": ["included", "details", "details"], "footer": ["footer", "details", "details"], "motto": ["motto", "details", "details"], "napkin": ["napkin", "details", "details"]};
const semanticRoles={...Object.fromEntries(Object.entries(native).map(([id,semanticRole])=>[id,{semanticRole}])),...Object.fromEntries(Object.entries(companions).map(([id,[semanticRole,panel,uiField]])=>[id,{semanticRole,text:id==='detailsLabel'?'detailsLabel':`sipAndPaint_${id}`,panel,uiField,moveTarget:panel,mappedControls:true,editable:true}]))};
const fontMap={Sip:'Sunset Paint Brush PNG',Paint:'Paint Splash Serif PNG',Brush:'Drift Brush SVG',TieSerif:'Didot',Script:'Dear Script (Demo_Font)',Sans:'LEMONMILK-Light',...Object.fromEntries(Object.keys(semanticRoles).map(id=>[id,id==='headline'?'Sunset Paint Brush PNG':id==='subtitle'?'Paint Splash Serif PNG':id==='and'?'Drift Brush SVG':id==='motto'?'Dear Script (Demo_Font)':['date','cost','venue'].includes(id)?'Didot':'LEMONMILK-Light']))};
export const cocoCssMasterAdapter={
 id:'sip-and-paint',masterPath:new URL('../public/generated-flyers/sip-and-paint-master.html',import.meta.url),outputPath:new URL('../public/generated-flyers/sip-and-paint.nflyer',import.meta.url),publicRoot:new URL('../public/',import.meta.url),editorTextScale:.5,requiredFonts:['Sip','Paint','Brush','TieSerif','Script','Sans'],requiredRoles:Object.values(native),semanticRoles,fontMap,
 recipe:{id:'sip-and-paint',name:'Sip and Paint — The Social Lounge',version:2,runtime:{compositionPattern:'center-poster-stack',styleId:'black-electric',palette:{bgFrom:'#070707',bgTo:'#171714',primary:'#f9f6eb',secondary:'#aaa69b',accent:'#dcc38e',neutral:'#edece5'},authority:{layout:{owner:'recipe-elements'},palette:{owner:'recipe-element-paint'},assets:{owner:'recipe-elements'},crop:{owner:'recipe-background-image'}},formats:{square:{canvas:{width:1080,height:1080}},story:{canvas:{width:1080,height:1920}}}}},eventBrief:{eventName:'SIP',subtitle:'PAINT',date:'APR 20'}
};
if(await access(new URL('../lib/template-data/sip-and-paint-saved-source.json',import.meta.url)).then(()=>true,()=>false))throw Error('Sip and Paint has an accepted user save; preserve it instead of rebuilding.');
...
for(const v of [result.square,result.story]){
 ...
 const bounds={x:5.7,y:v===result.story?17:18.5,width:30,height:2};
...
await writeFile(new URL('../lib/template-data/sip-and-paint-v2.json',import.meta.url),JSON.stringify({square:result.square,story:result.story},null,2)+'\n');
