...
// Full run: NF_YCEE_LIVE_EXPORT_ACCESS_FIXTURE=1 node scripts/verify-ycee-live.mjs
// Artwork iteration: NF_YCEE_LIVE_CONTROLS_CHECK=0 NF_YCEE_LIVE_EXPORT_CHECK=0 ...
// Use NF_YCEE_LIVE_FORMATS=square or story for a focused artwork iteration.
const out = process.env.NF_YCEE_LIVE_AUDIT_DIR || '/tmp/ycee-live';
...
const runControls = process.env.NF_YCEE_LIVE_CONTROLS_CHECK !== '0';
const runExports = process.env.NF_YCEE_LIVE_EXPORT_CHECK !== '0';
const runOriginalExports = runExports && process.env.NF_YCEE_LIVE_ORIGINAL_EXPORT_CHECK !== '0';
...
try {
  ...
  for (const format of formats) {
    ...
    assert.match(font, /Spotlight Gold PNG/, 'YCEE uses the supplied glyph font');
    assert.ok(await page.evaluate(()=>[...document.fonts].some(f=>f.family.replaceAll('\"','')==='Spotlight Gold PNG' && f.status==='loaded')), 'Actual gold font resource loaded, not just a CSS family name');
  ...
  if (runControls) {
    ...
    for (const format of formats) {
      await ready(format, initialMaps[format]);
      const checkObjects=process.env.NF_YCEE_LIVE_CHECK_OBJECTS?.split(',');
      for (const id of initialIds.filter(id=>!checkObjects||checkObjects.includes(id))) {
        ...
        if (['contactLabel', 'presents'].includes(id)) {
