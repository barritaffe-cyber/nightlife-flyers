...
try{
 ...
 for(const [id,file] of [['punta-cana','punta-cana.nflyer']]){
   ...
   if(!process.env.NF_ROUNDTRIP_ONLY) for(const format of ['square','story']){
     ...
     assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).replace(/\s/g,'').includes('Punta'));
     assert.equal(await page.locator('[data-coco-compiled-object="subtitle"]').count(),1);
     assert.equal(await page.locator('[data-sunset-foil-mask] mask').count(),4,'Cana uses four live SVG letter masks');
     assert.equal(await page.locator('[data-cana-metallic-material]').count(),4,'Every Cana letter receives the metallic surface');
     await page.locator('[data-cana-metallic-material]').first().evaluate(async el=>{const img=new Image();img.src=el.getAttribute('href');await img.decode();});
     assert.match(await page.locator('[data-sunset-foil-mask] [data-headline-shadow-glyph] rect').first().getAttribute('fill'),/^url\(/,'Material face is not overridden by transparent text paint');
     assert.equal(await page.locator('#artboard [data-node="time"]').count(),0,'No legacy duplicate doors label');
     ...
     await page.waitForFunction((format)=>{
       ...
       return board?.textContent.replace(/\s/g,'').includes('Punta') && !/Preparing (?:story|square) canvas/.test(board.textContent) && rect && (format==='story'?rect.height/rect.width>1.7:Math.abs(rect.height/rect.width-1)<0.05);
     ...
     if(process.env.NF_MASK_DETAIL)await page.locator('[data-sunset-foil-mask]').screenshot({path:`${out}/cana-${format}-material-detail.png`});
     if(!process.env.NF_PREVIEW_ONLY)for(const objectId of ["headline", "subtitle", "sundays", "date", "month", "genres", "tropical", "dj1", "dj2", "venue", "dress", "offers"]){
       ...
       if(!hit)console.log('HIT DIAGNOSTIC',await owner.evaluate(el=>({html:el.outerHTML.slice(0,7000),rect:el.getBoundingClientRect().toJSON(),surfaces:Array.from(el.querySelectorAll('[data-text-hit-surface]')).map(g=>({rect:g.getBoundingClientRect().toJSON(),hit:document.elementFromPoint(g.getBoundingClientRect().x+g.getBoundingClientRect().width/2,g.getBoundingClientRect().y+g.getBoundingClientRect().height/2)?.outerHTML.slice(0,600)}))})));
 ...
 const original=await input.inputValue();await input.fill('SUN TEST');await page.waitForTimeout(400);assert.ok((await owner.textContent()).includes('SUN TEST'));if(objectId==='subtitle')assert.equal((await owner.locator('mask text').allTextContents()).join(''),'SUN TEST','Mask lettering follows edited text');await input.fill(original);await page.waitForTimeout(400);
 }
 if(objectId==='subtitle')assert.equal(await owner.locator('mask').count(),4,'SVG masks restored after title edits');
 if(['genres','venue'].includes(objectId)){
       ...
       if(objectId==='genres'){
         ...
         assert.ok(Math.abs(tracking-(.32))<.001,format+' preserves authored details tracking: '+tracking);
 ...
 await page.waitForTimeout(2000);
 await page.locator('[aria-label="Preparing flyer canvas"]').waitFor({state:'hidden',timeout:90000});
 assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).includes('Punta'));
 assert.equal((await page.locator('[data-coco-compiled-object="genres"]').getAttribute('data-coco-compiled-align')).replace('start','left'),'left');
 ...
 for(const format of ['Square','Story']){
   ...
   await page.waitForFunction((format)=>{
     ...
     const ready=box && (format==='Story'?box.height/box.width>1.7:Math.abs(box.height/box.width-1)<.05) && Math.abs(tracking-(.32))<.001;
   ...
   await page.screenshot({clip:await page.locator('#artboard').boundingBox(),path:`${out}/punta-cana-${format.toLowerCase()}-roundtrip.png`});
 }
 if(process.env.NF_EXPORT_CHECKS){
   ...
   for(const format of ['Square','Story']){
     ...
     await writeFile(`${out}/punta-cana-${format.toLowerCase()}-export.png`,Buffer.from(data));
