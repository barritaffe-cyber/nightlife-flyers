...
const out=process.env.NF_COCO_AUDIT_DIR || '/tmp/coco-dress-code';
await mkdir(out,{recursive:true});
const browser=await chromium.launch({headless:true});
try {
 for(const [recipe,eventName,field,answer,owner,expected] of [
  ['glow-in-the-dark','Glow in the Dark','dressCode','white + neon','genres','WEAR WHITE + NEON\nLET THE NIGHT SHINE'],
  ['pulse','Pulse Sunday','eventDetails','Enjoy great food, good music and friends all night','details','ENJOY GREAT FOOD, GOOD MUSIC AND FRIENDS ALL NIGHT']
 ]) {
  ...
  try {
   ...
   assert.equal(await inputs.count(),1,'user supplies one answer; labels and the motto are automatic');
   if(field==='eventDetails')assert.equal(await inputs.first().getAttribute('maxlength'),'50');
   await inputs.first().fill(answer);
   assert.equal(await inputs.first().inputValue(),answer,'no answer truncation');
   if(field==='dressCode')assert.ok((await page.getByTestId('coco-build-brief-dressCode-preview').textContent()).includes(expected));
