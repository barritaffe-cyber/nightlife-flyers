...
// Full run: NF_HONEY_NIGHTS_EXPORT_ACCESS_FIXTURE=1 node scripts/verify-honey-nights.mjs
// Artwork iteration: NF_HONEY_NIGHTS_CONTROLS_CHECK=0 NF_HONEY_NIGHTS_EXPORT_CHECK=0 ...
// Use NF_HONEY_NIGHTS_FORMATS=square or story for a focused artwork iteration.
const out = process.env.NF_HONEY_NIGHTS_AUDIT_DIR || '/tmp/honey-nights';
const projectPath = process.env.NF_HONEY_NIGHTS_IMPORT || 'public/generated-flyers/honey-nights.nflyer';
const baseUrl = process.env.NF_BASE_URL || 'http://localhost:3000';
const runControls = process.env.NF_HONEY_NIGHTS_CONTROLS_CHECK !== '0';
const runExports = process.env.NF_HONEY_NIGHTS_EXPORT_CHECK !== '0';
const runOriginalExports = runExports && process.env.NF_HONEY_NIGHTS_ORIGINAL_EXPORT_CHECK !== '0';
const formats = (process.env.NF_HONEY_NIGHTS_FORMATS || 'square,story').split(',');
...
const initialIds = ['presenter','presents','headline','subtitle','mood','motto','day','month','date','time','age','tagline','djLabel','dj1','genres','venue','address'];
...
if (process.env.NF_HONEY_NIGHTS_EXPORT_ACCESS_FIXTURE === '1') {
...
async function ready(format, expectedMap) {
  ...
  await page.waitForFunction(({format, expectedMap}) => {
    ...
    if (!signature || window.__honeyNightsReadySignature !== signature) {
      window.__honeyNightsReadySignature = signature;
      window.__honeyNightsReadyAt = performance.now();
      return false;
    }
    return performance.now() - window.__honeyNightsReadyAt > 1800;
...
async function capture(name, publishFormat) {
  ...
  if (publishFormat && process.env.NF_HONEY_NIGHTS_PUBLISH_PREVIEWS === '1') {
    await board.screenshot({path: `public/generated-flyers/honey-nights-${publishFormat}-preview.png`, animations: 'disabled'});
  }
...
async function selectText(id) {
  ...
  const points = await target.evaluate(element => [...element.querySelectorAll('[data-text-hit-surface="true"]')].flatMap(surface => {
    ...
    return [.2, .35, .5, .65, .8].flatMap(y => Array.from({length:25},(_,i)=>(i+.5)/25).map(x => ({x: bounds.x + bounds.width * x, y: bounds.y + bounds.height * y})));
  }).filter(point => document.elementFromPoint(point.x, point.y)?.closest('[data-coco-compiled-object]') === element));
  if(!points.length)console.log('OCCLUSION',await target.evaluate(el=>[...el.querySelectorAll('[data-text-hit-surface]')].slice(0,3).map(n=>{const r=n.getBoundingClientRect();return {rect:r.toJSON(),stack:document.elementsFromPoint(r.x+r.width/2,r.y+r.height/2).slice(0,5).map(e=>e.outerHTML.slice(0,600))}})));
  for (const point of points) {
    await page.mouse.click(point.x, point.y);
    await page.waitForTimeout(40);
    if (await target.getAttribute('data-active') === 'true') break;
  }
  if(await target.getAttribute('data-active') !== 'true') console.log('HIT DIAGNOSTIC',id,points.length,await target.evaluate(el=>({html:el.outerHTML.slice(0,4000),rect:el.getBoundingClientRect().toJSON()})));
...
try {
  ...
  for (const format of formats) {
    ...
    assert.match(font, /Honey Gold Serif PNG/, 'HONEY uses the supplied glyph font');
    assert.ok(await page.evaluate(()=>[...document.fonts].some(f=>f.family.replaceAll('\"','')==='Honey Gold Serif PNG' && f.status==='loaded')), 'Actual gold font resource loaded, not just a CSS family name');
  ...
  if (runControls) {
    ...
    for (const format of formats) {
      await ready(format, initialMaps[format]);
      const checkObjects=process.env.NF_HONEY_NIGHTS_CHECK_OBJECTS?.split(',');
      for (const id of initialIds.filter(id=>!checkObjects||checkObjects.includes(id))) {
        ...
        if (['djLabel', 'presents'].includes(id)) {
