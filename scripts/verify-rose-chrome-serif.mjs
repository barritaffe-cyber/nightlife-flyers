...
const glyphFamily='Rose Chrome Serif PNG';
const sampleText = 'Rose\nCode';
const out = process.env.NF_OUTPUT_DIR || '/tmp/rose-chrome-serif-check';
...
try{
 await page.goto(`${baseUrl}/generated-flyers/rose-chrome-serif-font-preview.html`);
 ...
 for(const [id,file] of [['glyph-check',process.env.NF_PROJECT || 'day-party.nflyer']]){
   ...
   assert.equal(errors.length,0,JSON.stringify(errors));console.log('ROSE CHROME: BOTH PICKERS, FORMATS, EDITING, ROUNDTRIP, AND PNG EXPORT PASSED');
 }
