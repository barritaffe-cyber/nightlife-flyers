...
try{
 ...
 for(const [id,file] of [['zona-de-perreo','zona-de-perreo-updated.nflyer']]){
   ...
   for(const format of ['square','story']){
     ...
     const chooser=page.getByRole('button',{name:format==='square'?'Choose Square format':'Choose Story format',exact:true});
     ...
     const saved=JSON.parse(await readFile(`public/generated-flyers/${file}`,'utf8')).state.session[format];
     const expected=saved.cocoCompositionSystem.compiledObjectOverrides?.headline?.text ?? saved.headline;
     assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).includes(expected));
     ...
     await page.locator('#artboard').screenshot({path:`${out}/${id}-${format}-v3.png`});
