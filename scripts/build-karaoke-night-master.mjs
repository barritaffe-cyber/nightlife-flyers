...
const companions={"mood": ["mood", "details", "details"], "tagline": ["tagline", "details", "details"], "day": ["day", "date", "date"], "hours": ["hours", "date", "date"], "age": ["age", "details", "details"], "qrCaption": ["qrCaption", "details", "details"]};
const semanticRoles={...Object.fromEntries(Object.entries(native).map(([id,semanticRole])=>[id,{semanticRole}])),...Object.fromEntries(Object.entries(companions).map(([id,[semanticRole,panel,uiField]])=>[id,{semanticRole,text:id==='detailsLabel'?'detailsLabel':`karaokeNight_${id}`,panel,uiField,moveTarget:panel,mappedControls:true,editable:true}]))};
const fontMap={TieSerif:'Didot',Script:'Brittany Signature',Condensed:'Bebas Neue',Sans:'LEMONMILK-Light',...Object.fromEntries(Object.keys(semanticRoles).map(id=>[id,id==='subtitle'?'Brittany Signature':['headline','date','venue'].includes(id)?'Didot':id==='age'?'Bebas Neue':'LEMONMILK-Light']))};
export const cocoCssMasterAdapter={
 id:'karaoke-night',masterPath:new URL('../public/generated-flyers/karaoke-night-master.html',import.meta.url),outputPath:new URL('../public/generated-flyers/karaoke-night.nflyer',import.meta.url),publicRoot:new URL('../public/',import.meta.url),editorTextScale:.5,requiredFonts:['TieSerif','Sans','Script','Condensed'],requiredRoles:Object.values(native),semanticRoles,fontMap,
 recipe:{id:'karaoke-night',name:'Karaoke Night — Neon Lounge',version:2,runtime:{compositionPattern:'center-poster-stack',styleId:'black-electric',palette:{bgFrom:'#070707',bgTo:'#171714',primary:'#f9f6eb',secondary:'#aaa69b',accent:'#dcc38e',neutral:'#edece5'},authority:{layout:{owner:'recipe-elements'},palette:{owner:'recipe-element-paint'},assets:{owner:'recipe-elements'},crop:{owner:'recipe-background-image'}},formats:{square:{canvas:{width:1080,height:1080}},story:{canvas:{width:1080,height:1920}}}}},eventBrief:{eventName:'KARAOKE',subtitle:'Night',date:'DEC 1'}
};
if(await access(new URL('../lib/template-data/karaoke-night-saved-source.json',import.meta.url)).then(()=>true,()=>false))throw Error('Karaoke Night has an accepted user save; preserve it instead of rebuilding.');
...
for(const v of [result.square,result.story]){
 Object.assign(v,{backgroundUrl:blank,bgUrl:blank,subtagEnabled:false,rightRailEnabled:false,rightRailLabel:'',presenterEnabled:true,priceEnabled:false,priceLabel:'',qrEnabled:true,qrImageUrl:null,qrX:v===result.story?83:88,qrY:v===result.story?92:87,qrScale:7/11,leftRailEnabled:false,leftRail:'',leftRailLabel:'',headShadow:false,detailsShadow:false,venueShadow:false,socialHandleEnabled:false,cocoSocialHandleEnabled:false,details2Enabled:false,details2:'',djLineupLabel:'',exp:1,contrast:1/.9,saturation:1,warmth:0,tint:0,gamma:1,grain:0,vibrance:0,filmGrade:0});
...
await writeFile(new URL('../lib/template-data/karaoke-night-v2.json',import.meta.url),JSON.stringify({square:result.square,story:result.story},null,2)+'\n');
