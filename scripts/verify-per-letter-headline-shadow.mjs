...
try{
 ...
 for(const [id,file] of [['branch-happy-hour','branch-happy-hour.nflyer']]){
   ...
   if(!process.env.NF_ROUNDTRIP_ONLY) for(const format of ['square','story']){
     ...
     const result=await page.locator('[data-coco-glass-headline]').evaluate(el=>({
       groupFilter:getComputedStyle(el).filter,
       glyphs:Array.from(el.querySelectorAll('[data-headline-shadow-glyph]')).map(g=>({filter:getComputedStyle(g).filter,layers:g.querySelectorAll('[data-coco-glass-layer]').length})),
     }));
     assert.equal(result.groupFilter,'none');assert.equal(result.glyphs.length,5);
     for(const glyph of result.glyphs){assert.match(glyph.filter,/drop-shadow/);assert.equal(glyph.layers,4);}
     console.log('PER LETTER GLASS SHADOW PASSED',format);

   }
 }
 assert.deepEqual(errors, []);
 console.log('PER LETTER SHADOW CHECKS PASSED');
