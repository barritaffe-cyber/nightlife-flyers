...
const out = process.env.NF_OUTPUT_DIR || (process.env.NF_ACCEPTED_PREVIEW ? 'public/generated-flyers' : '/tmp/afrobeat-effects');
 ...
 for(const [id,file] of [['afrobeat-rooftop','afrobeat-rooftop.nflyer']]){
   ...
   for(const format of ['Square','Story']){
     ...
     for(const [objectId,panel] of [['headline','headline'],['subtitle','head2']]){
       ...
       await owner.evaluate(el=>el.click());await page.waitForTimeout(400);
       ...
       for(const [label,increment,property] of [['Size',4,'fontSize'],['Spacing',.02,'letterSpacing'],['Leading',.12,'lineHeight']]){
         ...
         await field.fill(String(Number(original)+increment));await field.press('Enter');await page.waitForTimeout(350);
