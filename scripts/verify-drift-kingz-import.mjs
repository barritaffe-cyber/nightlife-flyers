...
try{
 ...
 for(const [id,file] of [['drift-kingz','drift-kingz.nflyer']]){
   ...
   if(!process.env.NF_ROUNDTRIP_ONLY) for(const format of ['square','story']){
     ...
     if(!process.env.NF_PREVIEW_ONLY)for(const objectId of ["presenter", "headline", "subtitle", "day", "month", "date", "year", "genres", "description", "venue", "address", "mood", "benefits", "entry", "live"]){
 ...
 for(const format of ['Square','Story']){
   ...
   await page.screenshot({clip:await page.locator('#artboard').boundingBox(),path:`${out}/drift-kingz-${format.toLowerCase()}-roundtrip.png`});
 }
 if(process.env.NF_EXPORT_CHECKS){
   ...
   for(const format of ['Square','Story']){
     ...
     await writeFile(`${out}/drift-kingz-${format.toLowerCase()}-export.png`,Buffer.from(data));
