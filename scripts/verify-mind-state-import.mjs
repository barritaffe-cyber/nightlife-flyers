...
try{
 ...
 for(const [id,file] of [['mind-state','mind-state.nflyer']]){
   ...
   if(!process.env.NF_ROUNDTRIP_ONLY) for(const format of ['square','story']){
     ...
     assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).replace(/\s/g,'').includes('MIND'));
     ...
     await page.waitForFunction((format)=>{
       ...
       return board?.textContent.replace(/\s/g,'').includes('MIND') && !/Preparing (?:story|square) canvas/.test(board.textContent) && rect && (format==='story'?rect.height/rect.width>1.7:Math.abs(rect.height/rect.width-1)<0.05);
     ...
     if(!process.env.NF_PREVIEW_ONLY)for(const objectId of ["presenter", "headline", "subtitle", "day", "date", "year", "genres", "description", "venue", "address", "lineup", "handle", "age", "parking", "qrCaption"]){
 ...
 if(['genres','address'].includes(objectId)){
       ...
       if(objectId==='genres'){
         ...
         assert.ok(Math.abs(tracking-.35)<.001,format+' preserves authored details tracking: '+tracking);
 ...
 await page.waitForTimeout(2000);
 await page.locator('[aria-label="Preparing flyer canvas"]').waitFor({state:'hidden',timeout:90000});
 assert.ok((await page.locator('[data-coco-compiled-object="headline"]').textContent()).includes('MIND'));
 ...
 for(const format of ['Square','Story']){
   ...
   await page.waitForFunction((format)=>{
     ...
     const ready=box && (format==='Story'?box.height/box.width>1.7:Math.abs(box.height/box.width-1)<.05) && Math.abs(tracking-.35)<.001;
   ...
   await page.screenshot({clip:await page.locator('#artboard').boundingBox(),path:`${out}/mind-state-${format.toLowerCase()}-roundtrip.png`});
 }
 if(process.env.NF_EXPORT_CHECKS){
   ...
   for(const format of ['Square','Story']){
     ...
     await writeFile(`${out}/mind-state-${format.toLowerCase()}-export.png`,Buffer.from(data));
