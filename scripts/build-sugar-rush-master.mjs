...
const companions={"day": ["day", "date", "date"], "year": ["year", "date", "date"], "mood": ["mood", "details", "details"], "offer": ["offer", "details", "details"], "offerTime": ["offerTime", "details", "details"], "dj1": ["dj1", "details", "details"], "dj2": ["dj2", "details", "details"], "dj3": ["dj3", "details", "details"], "reservation": ["reservationInfo", "details", "details"], "vibes": ["vibes", "details", "details"], "qrCaption": ["qrCaption", "details", "details"], "detailsLabel": ["detailsLabel", "details", "detailsLabel"]};
const semanticRoles={...Object.fromEntries(Object.entries(native).map(([id,semanticRole])=>[id,{semanticRole}])),...Object.fromEntries(Object.entries(companions).map(([id,[semanticRole,panel,uiField]])=>[id,{semanticRole,text:id==='detailsLabel'?'detailsLabel':`sugarRush_${id}`,panel,uiField,moveTarget:panel,mappedControls:true,editable:true}]))};
const fontMap={Chrome:'Ladies Neon Chrome PNG',Brush:'Drift Brush SVG',Condensed:'Bebas Neue',Sans:'LEMONMILK-Light',...Object.fromEntries(Object.keys(semanticRoles).map(id=>[id,id==='headline'?'Ladies Neon Chrome PNG':id==='subtitle'?'Drift Brush SVG':['day','date','offer','venue','dj1','dj2','dj3'].includes(id)?'Bebas Neue':'LEMONMILK-Light']))};
export const cocoCssMasterAdapter={
 id:'sugar-rush',masterPath:new URL('../public/generated-flyers/sugar-rush-master.html',import.meta.url),outputPath:new URL('../public/generated-flyers/sugar-rush.nflyer',import.meta.url),publicRoot:new URL('../public/',import.meta.url),editorTextScale:.5,requiredFonts:['Chrome','Brush','Condensed','Sans'],requiredRoles:Object.values(native),semanticRoles,fontMap,
 recipe:{id:'sugar-rush',name:'Sugar Rush — Suite 5',version:2,runtime:{compositionPattern:'center-poster-stack',styleId:'black-electric',palette:{bgFrom:'#070707',bgTo:'#171714',primary:'#f9f6eb',secondary:'#aaa69b',accent:'#dcc38e',neutral:'#edece5'},authority:{layout:{owner:'recipe-elements'},palette:{owner:'recipe-element-paint'},assets:{owner:'recipe-elements'},crop:{owner:'recipe-background-image'}},formats:{square:{canvas:{width:1080,height:1080}},story:{canvas:{width:1080,height:1920}}}}},eventBrief:{eventName:'SUGAR',subtitle:'RUSH',date:'DEC 1'}
};
if(await access(new URL('../lib/template-data/sugar-rush-saved-source.json',import.meta.url)).then(()=>true,()=>false))throw Error('Sugar Rush has an accepted user save; preserve it instead of rebuilding.');
...
for(const v of [result.square,result.story]){
 Object.assign(v,{backgroundUrl:blank,bgUrl:blank,subtagEnabled:false,rightRailEnabled:true,rightRailLabel:'',presenterEnabled:true,priceEnabled:false,priceLabel:'',qrEnabled:true,qrImageUrl:null,qrX:88,qrY:v===result.story?93:87,qrScale:6.5/11,leftRailEnabled:false,leftRail:'',leftRailLabel:'',headShadow:false,detailsShadow:false,venueShadow:false,socialHandleEnabled:false,cocoSocialHandleEnabled:false,details2Enabled:false,details2:'',djLineupLabel:'',exp:1,contrast:1/.9,saturation:1,warmth:0,tint:0,gamma:1,grain:0,vibrance:0,filmGrade:0});
 ...
 const bounds={x:4.7,y:v===result.story?52:28,width:45,height:2};
...
await writeFile(new URL('../lib/template-data/sugar-rush-v2.json',import.meta.url),JSON.stringify({square:result.square,story:result.story},null,2)+'\n');
