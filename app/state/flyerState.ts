...
function sanitizeDesign(d: any, fallback: FlyerDesignV1): FlyerDesignV1 {
  // hard fallback if totally invalid
  if (!d || typeof d !== "object") return fallback;

  const safe: FlyerDesignV1 = {
    ...fallback,
    v: DESIGN_VERSION,
    savedAt: typeof d.savedAt === "string" ? d.savedAt : new Date().toISOString(),
  };

  if (isFormat(d.format)) safe.format = d.format;

  if (d.textStyles && typeof d.textStyles === "object")
    safe.textStyles = d.textStyles as TextStyleGroup;

  if (typeof d.head2Color === "string") safe.head2Color = d.head2Color;

  // toggles
  if (d.detailsEnabled) safe.detailsEnabled = d.detailsEnabled;
  if (d.details2Enabled) safe.details2Enabled = d.details2Enabled;
  if (d.headline2Enabled) safe.headline2Enabled = d.headline2Enabled;
  if (d.subtagEnabled) safe.subtagEnabled = d.subtagEnabled;
  if (d.venueEnabled) safe.venueEnabled = d.venueEnabled;
  if (d.emojisEnabled) safe.emojisEnabled = d.emojisEnabled;
  ...
  for (const k of [
    ...
    if (typeof d[k] === "boolean") (safe as any)[k] = d[k];
  }
  for (const k of [
    "headShadowStrength",
    "head2ShadowStrength",
    ...
    if (typeof d[k] === "number" && Number.isFinite(d[k])) (safe as any)[k] = d[k];
  }

  // layers
  if (d.portraits && typeof d.portraits === "object") safe.portraits = d.portraits;
  if (d.emojis && typeof d.emojis === "object") safe.emojis = d.emojis;

  // template/session
  if (d.currentTemplate && typeof d.currentTemplate === "object")
    safe.currentTemplate = d.currentTemplate;
  if (d.session && typeof d.session === "object") safe.session = d.session;
  if (d.sessionDirty && typeof d.sessionDirty === "object") safe.sessionDirty = d.sessionDirty;

  // ui-ish
  if (typeof d.selectedPanel === "string" || d.selectedPanel === null) safe.selectedPanel = d.selectedPanel;
  if (typeof d.selectedPortraitId === "string" || d.selectedPortraitId === null) safe.selectedPortraitId = d.selectedPortraitId;
  safe.moveTarget = (d.moveTarget ?? null) as MoveTarget;

  // ✅ NEW
  if (typeof d.selectedEmojiId === "string" || d.selectedEmojiId === null) {
    safe.selectedEmojiId = d.selectedEmojiId;
  }
