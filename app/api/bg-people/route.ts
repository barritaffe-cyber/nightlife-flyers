...
const PEOPLE_ENDPOINT =
  process.env.REPLICATE_PEOPLE_ENDPOINT || "https://api.replicate.com/v1/predictions";
const PEOPLE_VERSION = process.env.REPLICATE_PEOPLE_VERSION;
...
export async function POST(req: Request) {
  try {
    ...
    if (!isHttpUrl(image)) {
      return NextResponse.json({ elements: [], error: "Image must be a public URL or data URL." });
    }
    if (!PEOPLE_VERSION) {
      return NextResponse.json({ elements: [], error: "Missing REPLICATE_PEOPLE_VERSION" });
    }
