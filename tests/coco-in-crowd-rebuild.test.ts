...
const variants=JSON.parse(readFileSync(new URL('../lib/template-data/in-crowd-v2.json',import.meta.url),'utf8'));
test('The In Crowd preserves both backgrounds, editable Classic Gold Serif and independent bindings',()=>{
 for(const [format,v] of Object.entries(variants) as [string,any][]){
  ...
  assert.equal(doc.objects.find((o:any)=>o.id==='background').image.src,`/generated-flyers/assets/in-crowd-${format}.jpg`);
  const title=doc.objects.find((o:any)=>o.id==='headline');
  assert.equal(title.typography.fontFamily,'Classic Gold Serif PNG');assert.equal(title.text,'IN');assert.equal(doc.objects.find((o:any)=>o.id==='prefix').text,'THE');assert.equal(doc.objects.find((o:any)=>o.id==='subtitle').text,'CROWD');
...
test('The In Crowd portable sessions equal gallery variants',()=>{
 const sessions=JSON.parse(readFileSync(new URL('../public/generated-flyers/in-crowd.nflyer',import.meta.url),'utf8')).state.session;
 ...
 assert.ok(hidden);assert.doesNotMatch(hidden,/'atlanta'/);
 assert.match(registry,/inCrowdTemplate\.preview = '\/generated-flyers\/in-crowd-square-preview.png'/);
});
