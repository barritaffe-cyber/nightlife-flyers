...
const variants=JSON.parse(readFileSync(new URL('../lib/template-data/mind-state-v2.json',import.meta.url),'utf8'));
test('Mind State preserves both backgrounds, editable Arctic Metal and independent bindings',()=>{
 for(const [format,v] of Object.entries(variants) as [string,any][]){
  ...
  assert.equal(doc.objects.find((o:any)=>o.id==='background').image.src,`/generated-flyers/assets/techno-${format}.jpg`);
  const title=doc.objects.find((o:any)=>o.id==='headline');
  assert.equal(title.typography.fontFamily,'Arctic Metal PNG');assert.equal(title.text,'MIND\nSTATE');
...
test('Mind State portable sessions equal gallery variants',()=>{
 const sessions=JSON.parse(readFileSync(new URL('../public/generated-flyers/mind-state.nflyer',import.meta.url),'utf8')).state.session;
 ...
 assert.ok(hidden);assert.doesNotMatch(hidden,/'edm_stage_co2'/);
 assert.match(registry,/mindStateTemplate\.preview = '\/generated-flyers\/mind-state-square-preview.png'/);
});
