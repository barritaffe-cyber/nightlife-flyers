...
const variants=JSON.parse(readFileSync(new URL('../lib/template-data/mojito-monday-v2.json',import.meta.url),'utf8'));
test('Mojito Monday preserves both backgrounds, case-sensitive Mojito lettering and independent bindings',()=>{
 for(const [format,v] of Object.entries(variants) as [string,any][]){
  ...
  assert.equal(doc.objects.find((o:any)=>o.id==='background').image.src,`/generated-flyers/assets/mojito-monday-${format}.jpg`);
  const title=doc.objects.find((o:any)=>o.id==='headline');
  assert.equal(title.typography.fontFamily,'Mojito Serif PNG');assert.equal(title.text,'MOJITo');assert.equal(doc.objects.find((o:any)=>o.id==='subtitle').text,'MONDAY');
...
test('Mojito Monday portable sessions equal gallery variants',()=>{
 const sessions=JSON.parse(readFileSync(new URL('../public/generated-flyers/mojito-monday.nflyer',import.meta.url),'utf8')).state.session;
 ...
 assert.ok(hidden);assert.doesNotMatch(hidden,/'kpop_pastel_led'/);
 assert.match(registry,/id: 'kpop_pastel_led'/);assert.match(registry,/mojitoMondayRebuiltTemplate\.preview = '\/generated-flyers\/mojito-monday-square-preview.png'/);
});
