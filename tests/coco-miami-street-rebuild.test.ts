...
const variants=JSON.parse(readFileSync(new URL('../lib/template-data/miami-street-v2.json',import.meta.url),'utf8'));
test('Miami Street Nights preserves supplied backgrounds and independent editable objects in both formats',()=>{
 for(const [format,v] of Object.entries(variants) as [string,any][]){
  ...
  assert.ok(Math.abs(v.head2Fx.tracking-1.6)<1e-8);
  assert.equal(doc.objects.length,17);
  ...
  assert.equal(doc.objects.find((o:any)=>o.id==='background').image.src,`/generated-flyers/miami-skline-${format}.jpg`);
  ...
  assert.equal(new Set(texts.map((o:any)=>o.binding.text)).size,11);
  ...
  assert.match(texts.find((o:any)=>o.id==='headline').paint.backgroundImage,/^url\("\/generated-flyers\/assets\/miami-street-title-texture.svg"\)$/);
 }
});
test('Miami Street Nights portable sessions equal gallery data',()=>{
 const sessions=JSON.parse(readFileSync(new URL('../public/generated-flyers/miami-street.nflyer',import.meta.url),'utf8')).state.session;
