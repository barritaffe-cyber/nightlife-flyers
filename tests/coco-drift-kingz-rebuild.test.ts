...
const variants=JSON.parse(readFileSync(new URL('../lib/template-data/drift-kingz-v2.json',import.meta.url),'utf8'));
test('Drift Kingz preserves both backgrounds, editable vector brush and independent bindings',()=>{
 for(const [format,v] of Object.entries(variants) as [string,any][]){
  ...
  assert.equal(doc.objects.find((o:any)=>o.id==='background').image.src,`/generated-flyers/assets/drift-kingz-${format}.jpg`);
  const title=doc.objects.find((o:any)=>o.id==='headline');
  assert.equal(title.typography.fontFamily,'Anton');assert.match(title.paint.backgroundImage,/^url\("\/generated-flyers\//);assert.doesNotMatch(title.paint.backgroundSize,/%/);assert.equal(doc.objects.find((o:any)=>o.id==='subtitle').typography.fontFamily,'Drift Brush SVG');assert.equal(title.text,'MIAMI');assert.equal(doc.objects.find((o:any)=>o.id==='subtitle').text,'DRIFT KINGZ');
...
test('Drift Kingz portable sessions equal gallery variants',()=>{
 const sessions=JSON.parse(readFileSync(new URL('../public/generated-flyers/drift-kingz.nflyer',import.meta.url),'utf8')).state.session;
 ...
 assert.ok(hidden);assert.doesNotMatch(hidden,/'miami_heat'/);
 assert.match(registry,/driftKingzTemplate\.preview = '\/generated-flyers\/drift-kingz-square-preview.png'/);
});
