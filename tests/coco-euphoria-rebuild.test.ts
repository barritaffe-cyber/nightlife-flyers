...
const variants=JSON.parse(readFileSync(new URL('../lib/template-data/euphoria-v2.json',import.meta.url),'utf8'));
test('Euphoria preserves both backgrounds, chrome lettering and independent bindings',()=>{
 for(const [format,v] of Object.entries(variants) as [string,any][]){
  ...
  assert.equal(doc.objects.find((o:any)=>o.id==='background').image.src,`/generated-flyers/assets/euphoria-${format}.jpg`);
  const title=doc.objects.find((o:any)=>o.id==='headline');
  assert.equal(title.typography.fontFamily,'Euphoria Chrome PNG');assert.equal(title.text,'EUPHORIA');assert.equal(doc.objects.find((o:any)=>o.id==='subtitle').text,'MUSIC LIVES HIGHER HERE');
...
test('Euphoria portable sessions equal gallery variants',()=>{
 const sessions=JSON.parse(readFileSync(new URL('../public/generated-flyers/euphoria.nflyer',import.meta.url),'utf8')).state.session;
 ...
 assert.ok(hidden);assert.doesNotMatch(hidden,/'edm_tunnel'/);
 assert.match(registry,/id: 'edm_tunnel'/);assert.match(registry,/euphoriaRebuiltTemplate\.preview = '\/generated-flyers\/euphoria-square-preview.png'/);
});
