...
const variants=JSON.parse(readFileSync(new URL('../lib/template-data/sugar-rush-v2.json',import.meta.url),'utf8'));
test('Sugar Rush preserves both backgrounds, editable neon chrome and independent bindings',()=>{
 for(const [format,v] of Object.entries(variants) as [string,any][]){
  ...
  assert.equal(doc.objects.find((o:any)=>o.id==='background').image.src,`/generated-flyers/assets/sugar-rush-${format}.jpg`);
  const title=doc.objects.find((o:any)=>o.id==='headline');
  assert.equal(title.typography.fontFamily,'Ladies Neon Chrome PNG');assert.equal(title.text,'SUGAR');assert.equal(doc.objects.find((o:any)=>o.id==='subtitle').text,'RUSH');
...
test('Sugar Rush portable sessions equal gallery variants',()=>{
 const sessions=JSON.parse(readFileSync(new URL('../public/generated-flyers/sugar-rush.nflyer',import.meta.url),'utf8')).state.session;
 ...
 assert.ok(hidden);assert.doesNotMatch(hidden,/'sugar_rush'/);
 assert.match(registry,/id: 'sugar_rush'/);assert.match(registry,/sugarRushRebuiltTemplate\.preview = '\/generated-flyers\/sugar-rush-square-preview.png'/);
});
