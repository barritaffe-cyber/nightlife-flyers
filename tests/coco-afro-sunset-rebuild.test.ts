...
const variants=JSON.parse(readFileSync(new URL('../lib/template-data/afro-sunset-v2.json',import.meta.url),'utf8'));
test('Afro Sunset preserves both backgrounds, cream and gold typography and independent bindings',()=>{
 for(const [format,v] of Object.entries(variants) as [string,any][]){
  ...
  assert.equal(doc.objects.find((o:any)=>o.id==='background').image.src,`/generated-flyers/assets/afro-sunset-${format}.jpg`);
  const title=doc.objects.find((o:any)=>o.id==='headline');
  assert.equal(title.typography.fontFamily,'Didot');assert.equal(title.text,'AFRO');assert.equal(doc.objects.find((o:any)=>o.id==='subtitle').text,'SUNSET');
...
test('Afro Sunset portable sessions equal gallery variants',()=>{
 const sessions=JSON.parse(readFileSync(new URL('../public/generated-flyers/afro-sunset.nflyer',import.meta.url),'utf8')).state.session;
 ...
 assert.ok(hidden);assert.doesNotMatch(hidden,/'luxe'/);
 assert.match(registry,/id: 'luxe'/);assert.match(registry,/afroSunsetRebuiltTemplate\.preview = '\/generated-flyers\/afro-sunset-square-preview.png'/);
});
