...
const variants=JSON.parse(readFileSync(new URL('../lib/template-data/nocturne-muse-v2.json',import.meta.url),'utf8'));
test('Nocturne Midnight Muse preserves both backgrounds, editable rose serif and independent bindings',()=>{
 for(const [format,v] of Object.entries(variants) as [string,any][]){
  ...
  assert.equal(doc.objects.find((o:any)=>o.id==='background').image.src,`/generated-flyers/assets/nockturn-${format}.jpg`);
  const title=doc.objects.find((o:any)=>o.id==='headline');
  assert.equal(title.typography.fontFamily,'Didot');assert.equal(title.text,'NOCTURNE');assert.equal(doc.objects.find((o:any)=>o.id==='subtitle').text,'Midnight Muse');
...
test('Nocturne Midnight Muse portable sessions equal gallery variants',()=>{
 const sessions=JSON.parse(readFileSync(new URL('../public/generated-flyers/nocturne-muse.nflyer',import.meta.url),'utf8')).state.session;
 ...
 assert.ok(hidden);assert.doesNotMatch(hidden,/'nocturne_midnight_muse'/);
 assert.match(registry,/id: 'nocturne_midnight_muse'/);assert.match(registry,/preview: '\/generated-flyers\/nocturne-muse-square-preview.png'/);
});
