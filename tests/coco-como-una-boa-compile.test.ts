...
const project=JSON.parse(readFileSync('public/generated-flyers/como-una-boa.nflyer','utf8'));
for(const format of ['square','story'])test(`Como Una Boa ${format} has portable fonts and editable sidebar text`,()=>{
 ...
 for(const o of text){assert.ok(o.editable);assert.ok(o.binding.panel,o.id);assert.ok(o.binding.text,o.id);assert.doesNotMatch(o.typography.fontFamily,/^Boa/,'CSS-only aliases must not reach the editor');}
 assert.equal(text.find((o:any)=>o.id==='headline').typography.fontFamily,'Anton');
...
test('Como Una Boa keeps distinct supplied backgrounds for Square and Story',()=>{
