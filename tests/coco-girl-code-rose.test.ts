...
const id='girl-code-rose';
const formats=JSON.parse(readFileSync('public/generated-flyers/girl-code-rose.nflyer','utf8')).state.session;
const caps=cocoRecipeFormCapabilities(id,formats);
const variants=['square','story'] as const;
test('Rose Chrome Serif exposes all 62 supplied characters as reusable lettering',()=>{
 const family='Rose Chrome Serif PNG';
 const folder='public/generated-flyers/assets/png-glyphs/rose-chrome-serif';
 ...
 for(const [char,glyph] of Object.entries(metrics.glyphs) as [string,any][]){
  const png=readFileSync(`${folder}/${glyph.nativeGlyph}`);
  assert.equal(png.subarray(1,4).toString(),'PNG',char);
  assert.ok(glyph.advance>0,char);
 }
 assert.ok(PNG_GLYPH_COLLECTIONS.some(font=>font.family===family));
 assert.ok(isPngGlyphFamily(family));
 assert.equal(TEMPLATE_ONLY_FONT_FAMILIES.has(family),false);
 const font=readFileSync(`public${FONT_FILE_MAP[family].split('?')[0]}`);
 assert.equal(font.subarray(0,4).toString(),'wOF2');
});
test('Girl Code Rose registers both authored formats with its own assets',()=>{
 assert.equal(getVisualRecipe(id)?.name,'Girl Code — Rose');
 assert.equal(COCO_PORTABLE_RECIPE_PROJECT_URLS[id],'/generated-flyers/girl-code-rose.nflyer');
 assert.deepEqual(COCO_RECIPE_CATALOG[id].themes,['R&B / Lounge','Tropical','Elegant']);
 ...
 for(const format of variants){
  ...
  assert.equal(doc.objects.find((o:any)=>o.id==='background').image.src,`/generated-flyers/assets/girl-code-${format}2.jpg`);
  ...
  assert.equal(text.find((o:any)=>o.id==='headline').typography.fontFamily,'Rose Chrome Serif PNG');
  assert.ok(text.find((o:any)=>o.id==='subtitle').stacking.zIndex>text.find((o:any)=>o.id==='headline').stacking.zIndex,'script paints in front of the serif title');
 }
});
test('Form asks for date, time, three DJs, genres, venue and reservations independently',()=>{
 for(const [field,owners] of Object.entries({presenterName:['presenter'],date:['day','month','date'],startTime:['time'],endTime:['endTime'],djs:['dj1','dj2','dj3'],musicPolicy:['genres'],venueName:['venue'],address:['address'],rsvpContact:['contact'],'recipe:venueCaption':['venueCaption']})){
...
test('DJ and reservations labels hide with empty fields',()=>{
 for(const format of variants){
  ...
  assert.equal(edits.contactPlate?.removed,true,`${format}: empty reservation plate hides`);
 }
});
