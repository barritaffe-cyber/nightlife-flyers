...
const variants=JSON.parse(readFileSync(new URL('../lib/template-data/electric-sunset-v2.json',import.meta.url),'utf8'));
test('Electric Sunset preserves both backgrounds, cream and gold typography and independent bindings',()=>{
 for(const [format,v] of Object.entries(variants) as [string,any][]){
  ...
  assert.equal(doc.objects.find((o:any)=>o.id==='background').image.src,`/generated-flyers/assets/la-luxe-${format}.jpg`);
  const title=doc.objects.find((o:any)=>o.id==='headline');
  assert.equal(title.typography.fontFamily,'Galaxia Personal Used');assert.equal(title.typography.lineHeight,.72);assert.equal(title.text,'SUN\nSET');assert.equal(doc.objects.find((o:any)=>o.id==='subtitle').text,'ELECTRIC');
...
test('Electric Sunset portable sessions equal gallery variants',()=>{
 const sessions=JSON.parse(readFileSync(new URL('../public/generated-flyers/electric-sunset.nflyer',import.meta.url),'utf8')).state.session;
 ...
 assert.ok(hidden);assert.doesNotMatch(hidden,/'la-lux'/);
 assert.match(registry,/id: 'la-lux'/);assert.match(registry,/electricSunsetRebuiltTemplate\.preview = '\/generated-flyers\/electric-sunset-square-preview.png'/);
});

test('Electric Sunset uses the existing Tech / Futuristic Display font',()=>{
 const fonts=readFileSync(new URL('../lib/fonts.ts',import.meta.url),'utf8');
 assert.match(fonts,/label: \"Tech \/ Futuristic Display\"[\s\S]*?\"Galaxia Personal Used\"/);
});
