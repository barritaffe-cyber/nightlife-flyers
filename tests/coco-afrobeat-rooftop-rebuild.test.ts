...
const variants=JSON.parse(readFileSync(new URL('../lib/template-data/afrobeat-rooftop-v2.json',import.meta.url),'utf8'));
test('Afrobeat Rooftop preserves supplied backgrounds and independent editable objects in both formats',()=>{
 for(const [format,v] of Object.entries(variants) as [string,any][]){
  ...
  assert.ok(doc.objects.find((o:any)=>o.id==='africa').image.src.includes('afrobeat-africa.svg'));
  ...
  assert.equal(doc.objects.find((o:any)=>o.id==='background').image.src,`/generated-flyers/assets/afro-${format}.${format==='square'?'png':'jpg'}`);
  ...
  assert.match(texts.find((o:any)=>o.id==='headline').paint.backgroundImage,/^url\("\/generated-flyers\/assets\/afrobeat-rooftop-gold.svg"\)$/);
 }
});
test('Afrobeat Rooftop portable sessions equal gallery data',()=>{
 const sessions=JSON.parse(readFileSync(new URL('../public/generated-flyers/afrobeat-rooftop.nflyer',import.meta.url),'utf8')).state.session;
