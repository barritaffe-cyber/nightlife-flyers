...
test('Bad Girls has one Urban portrait direction, a portable source, and both preview routes', () => {
  assert.equal(getVisualRecipe(recipeId)?.version, 1);
  assert.equal(getVisualRecipe(recipeId)?.name, 'Bad Girls');
  assert.deepEqual(COCO_RECIPE_CATALOG[recipeId].themes, ['Urban', 'Reggae / Dancehall']);
  assert.equal(COCO_RECIPE_CATALOG[recipeId].containsSubject, true);
  assert.equal(COCO_PORTABLE_RECIPE_PROJECT_URLS[recipeId], '/generated-flyers/bad-girls.nflyer');
  const matched = directions.filter(direction => direction.visualRecipeId === recipeId);
  assert.equal(matched.length, 1);
  assert.equal(matched[0].subjectPolicy.mode, 'preferred-hero');
  assert.equal(matched[0].palettePolicy.id, 'deep-red-gold');
  assert.ok(matched[0].eligibleNightlifeStyles.includes('ladies-night'));
  assert.deepEqual(getCocoRecipePreviewExports(recipeId), {
    square: '/generated-flyers/bad-girls-square-preview.png',
    story: '/generated-flyers/bad-girls-story-preview.png',
  });
});

test('Bad Girls discovery respects its two title words and separate portrait', () => {
  const formats = savedFormats();
  const direction = directions.find(item => item.visualRecipeId === recipeId)!;
  assert.equal(isCocoRecipeChoiceEligible(direction, {}, formats), false);
  assert.equal(isCocoRecipeChoiceEligible(direction, { subjectDataUrl: '/user-portrait.png' }, formats), true);
  for (const name of ['Bad Girls', 'Neon Glow']) assert.equal(cocoHeadlineMatches(recipeId, formats, name), true);
  for (const name of ['Beat', 'Bad Girls Tonight']) assert.equal(cocoHeadlineMatches(recipeId, formats, name), false);
  ...
  const ranked = rankCocoMatchingDirections(directions, { eventName: 'Bad Girls', subjectDataUrl:'/user-portrait.png', eventBrief: { theme: 'Urban' } },
    id => templates.find((item: { recipeId: string }) => item.recipeId === id)?.formats);
  assert.equal(ranked[0]?.visualRecipeId, recipeId);
});

test('Bad Girls ships independent format artwork and individually editable text', () => {
  const formats = savedFormats();
  ...
  for (const format of ['square', 'story'] as const) {
    ...
    assert.equal(text.find((object: any) => object.id === 'headline')?.text, 'BAD');
    assert.equal(text.find((object: any) => object.id === 'subtitle')?.text, 'GIRLS');
    ...
    assert.equal(document.objects.some((object: any) => object.semanticRole === 'subject' || object.assetRole === 'subject'), true);
...
test('Bad Girls form keeps table availability, reservations, lineup, and title text independent', () => {
  const formats = savedFormats();
  const capabilities = cocoRecipeFormCapabilities(recipeId, formats);
  assert.deepEqual(capabilities.bindings['eventDetails'].targets, { square: ['tables'], story: ['tables'] });
  assert.equal(capabilities.bindings['eventDetails'].label, 'Table availability');
  assert.equal(cocoFormFieldGuidance('eventDetails', capabilities.bindings['eventDetails']), 'Add table info, like VIP Tables Available.');
  ...
  const brief = cocoBriefForCapabilities({ theme: 'Urban', 'eventDetails': 'vip seating\navailable', rsvpContact: '555 123 4567', djs: 'dj nova\ndj orbit', subtitle: 'great\nmusic' }, capabilities);
  for (const format of ['square', 'story'] as const) {
    ...
    assert.equal(edits.contact.text, 'RSVP 555 123 4567');
    assert.equal(edits.dj1.text, 'DJ NOVA');
    assert.equal(edits.dj2.text, 'DJ ORBIT');
    assert.equal(edits.tagline.text, 'Great\nMusic');
