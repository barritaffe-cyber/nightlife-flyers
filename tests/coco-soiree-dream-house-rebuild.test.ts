...
const variants=JSON.parse(readFileSync(new URL('../lib/template-data/soiree-dream-house-v2.json',import.meta.url),'utf8'));
test('Soiree Dream House preserves both backgrounds, cream and gold typography and independent bindings',()=>{
 for(const [format,v] of Object.entries(variants) as [string,any][]){
  ...
  assert.equal(doc.objects.find((o:any)=>o.id==='background').image.src,`/generated-flyers/assets/soiree-${format}.jpg`);
  const title=doc.objects.find((o:any)=>o.id==='headline');
  assert.equal(title.typography.fontFamily,'Dear Script (Demo_Font)');assert.equal(title.text,'Soiree');assert.match(title.paint.textShadow,/rgb/);assert.equal(doc.objects.find((o:any)=>o.id==='subtitle').typography.fontFamily,'Didot');assert.equal(doc.objects.find((o:any)=>o.id==='subtitle').text,'AN EVENING OF ELEGANCE\n& CELEBRATION');
...
test('Soiree Dream House portable sessions equal gallery variants',()=>{
 const sessions=JSON.parse(readFileSync(new URL('../public/generated-flyers/soiree-dream-house.nflyer',import.meta.url),'utf8')).state.session;
 ...
 assert.match(registry,/id:'soiree_dream_house'/);
 assert.match(registry,/Soiree — Dream House/);
});
