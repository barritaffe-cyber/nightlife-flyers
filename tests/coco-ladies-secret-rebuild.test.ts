...
const variants=JSON.parse(readFileSync(new URL('../lib/template-data/ladies-secret-v2.json',import.meta.url),'utf8'));
test('Ladies Night preserves both backgrounds, editable neon chrome and independent bindings',()=>{
 for(const [format,v] of Object.entries(variants) as [string,any][]){
  ...
  assert.equal(doc.objects.find((o:any)=>o.id==='background').image.src,`/generated-flyers/assets/secret-friday-${format}.jpg`);
  const title=doc.objects.find((o:any)=>o.id==='headline');
  assert.equal(title.typography.fontFamily,'Ladies Neon Chrome PNG');assert.equal(title.text,'LADIES');assert.equal(doc.objects.find((o:any)=>o.id==='subtitle').text,'NIGHT');
  ...
  assert.equal(texts.length,22);assert.equal(new Set(texts.map((o:any)=>o.binding.text)).size,22);
...
test('Ladies Night portable sessions equal gallery variants',()=>{
 const sessions=JSON.parse(readFileSync(new URL('../public/generated-flyers/ladies-secret.nflyer',import.meta.url),'utf8')).state.session;
 ...
 assert.ok(hidden);assert.doesNotMatch(hidden,/'secret_friday'/);
 assert.match(registry,/id: 'secret_friday'/);assert.match(registry,/ladiesSecretTemplate\.preview = '\/generated-flyers\/ladies-secret-square-preview.png'/);
});
