...
const variants=JSON.parse(readFileSync(new URL('../lib/template-data/miami-ocean-nights-v2.json',import.meta.url),'utf8'));
test('Miami Nights Ocean Drive preserves both backgrounds, cream and gold typography and independent bindings',()=>{
 for(const [format,v] of Object.entries(variants) as [string,any][]){
  ...
  assert.equal(doc.objects.find((o:any)=>o.id==='background').image.src,`/generated-flyers/assets/miami-${format}.jpg`);
  const title=doc.objects.find((o:any)=>o.id==='headline');
  assert.equal(title.typography.fontFamily,'Didot');assert.equal(title.text,'MIAMI');assert.equal(Number(title.typography.fontWeight),700);assert.equal(doc.objects.find((o:any)=>o.id==='subtitle').typography.fontFamily,'Dear Script (Demo_Font)');assert.equal(doc.objects.find((o:any)=>o.id==='subtitle').text,'Nights');
...
test('Miami Nights Ocean Drive portable sessions equal gallery variants',()=>{
 const sessions=JSON.parse(readFileSync(new URL('../public/generated-flyers/miami-ocean-nights.nflyer',import.meta.url),'utf8')).state.session;
 ...
 assert.match(registry,/id:'miami_ocean_nights'/);
 assert.match(registry,/Miami Nights — Ocean Drive/);
});
