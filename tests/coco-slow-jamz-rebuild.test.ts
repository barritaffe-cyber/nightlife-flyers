...
const variants=JSON.parse(readFileSync(new URL('../lib/template-data/slow-jamz-v2.json',import.meta.url),'utf8'));
test('Slow Jamz preserves both backgrounds, editable Classic Gold Serif and independent bindings',()=>{
 for(const [format,v] of Object.entries(variants) as [string,any][]){
  ...
  assert.equal(doc.objects.find((o:any)=>o.id==='background').image.src,`/generated-flyers/assets/r%26b-lounge-${format}.jpg`);
  const title=doc.objects.find((o:any)=>o.id==='headline');
  assert.equal(title.typography.fontFamily,'Classic Gold Serif PNG');assert.equal(title.text,'SLOW\nJAMZ');
...
test('Slow Jamz portable sessions equal gallery variants',()=>{
 const sessions=JSON.parse(readFileSync(new URL('../public/generated-flyers/slow-jamz.nflyer',import.meta.url),'utf8')).state.session;
 ...
 assert.ok(hidden);assert.doesNotMatch(hidden,/'rnb_velvet'/);
 assert.match(registry,/slowJamzTemplate\.preview = '\/generated-flyers\/slow-jamz-square-preview.png'/);
});
