...
const variants=JSON.parse(readFileSync(new URL('../lib/template-data/martini-night-v2.json',import.meta.url),'utf8'));
test('Martini Night preserves both backgrounds, editable serif and script and independent bindings',()=>{
 for(const [format,v] of Object.entries(variants) as [string,any][]){
  ...
  assert.equal(doc.objects.find((o:any)=>o.id==='background').image.src,`/generated-flyers/assets/martini-${format}.jpg`);
  const title=doc.objects.find((o:any)=>o.id==='headline');
  assert.equal(title.typography.fontFamily,'Didot');assert.equal(title.text,'MARTINI');assert.equal(doc.objects.find((o:any)=>o.id==='subtitle').text,'Night');
  ...
  assert.equal(texts.length,11);assert.equal(new Set(texts.map((o:any)=>o.binding.text)).size,11);
...
test('Martini Night portable sessions equal gallery variants',()=>{
 const sessions=JSON.parse(readFileSync(new URL('../public/generated-flyers/martini-night.nflyer',import.meta.url),'utf8')).state.session;
 ...
 assert.ok(hidden);assert.doesNotMatch(hidden,/'martini'/);
 assert.match(registry,/martiniNightTemplate\.preview = '\/generated-flyers\/martini-night-square-preview.png'/);
});
